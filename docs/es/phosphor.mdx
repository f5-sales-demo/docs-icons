...
description: >-
  Referencia seleccionada de Phosphor Icons — 9.161 iconos con variantes de peso
  flexibles disponibles mediante el componente Icon.
sidebar:
  label: Phosphor
  order: 8
i18n:
...
[Phosphor Icons](https://icon-sets.iconify.design/ph/) proporciona 9.161 iconos con opciones de peso flexibles — regular, bold, duotone, fill, light y thin. Utilice el componente `Icon` de `@f5-sales-demo/icons-phosphor` para renderizar cualquier icono por nombre.
...
Los iconos se renderizan como SVG en línea usando `currentColor` — se adaptan automáticamente al modo claro y oscuro.

:::note[Variantes de peso]
Los iconos Phosphor vienen en seis pesos. El peso regular utiliza el nombre base (p. ej. `globe`). Los demás pesos añaden un sufijo: `globe-bold`, `globe-duotone`, `globe-fill`, `globe-light`, `globe-thin`. Los ejemplos a continuación muestran el peso regular.
:::
...
Para la lista completa de 9.161 Phosphor Icons (en todas las variantes de peso), visite el [explorador de Iconify Phosphor](https://icon-sets.iconify.design/ph/).

...
El tema de documentación proporciona acceso a miles de iconos de múltiples paquetes. Todos los paquetes utilizan el mismo patrón: importar el componente `Icon.astro` del paquete y referenciar los iconos por nombre.
...
Los iconos utilizan `currentColor` y se adaptan automáticamente al modo claro y oscuro sin necesidad de un manejo especial.
...
Todos los componentes `Icon` renderizan SVGs en línea utilizando `currentColor`. Se adaptan automáticamente al tema actual — no se necesita manejo especial de CSS ni de clases.

Los iconos de servicios F5 XC utilizan propiedades personalizadas de CSS que se adaptan automáticamente al modo claro y oscuro — no se necesita manejo especial de clases. Para otros iconos multicolor (como las variantes de color de HashiCorp Flight), la clase `no-invert` en `.icon-card-image` preserva los colores originales en modo oscuro.
...
| Diagramas de productos F5 | F5 Brand | Arte específico de redes/seguridad del dominio |
...
| Lenguaje de diseño IBM | Carbon | 2,582 iconos de grado empresarial |
| Opciones flexibles de peso | Phosphor | 9,161 iconos con seis variantes de peso |
...
| Logos de marcas/empresas | Simple Icons | Más de 3,200 iconos de marcas para proveedores y servicios |

---
title: Iconos integrados de Starlight
description: >-
  Referencia completa de los más de 200 iconos integrados de Starlight
  disponibles a través del componente Icon.
sidebar:
  label: Integrados de Starlight
  order: 3
i18n:
...
Starlight incluye más de 200 iconos integrados disponibles a través del componente `<Icon>` — sin necesidad de instalación. El plugin `starlight-plugin-icons` ya está configurado en el tema, por lo que estos iconos funcionan en cualquier repositorio de contenido.
...
| `label` | `string` | — | Etiqueta accesible para lectores de pantalla. Úsela para iconos significativos; omítala para los decorativos. |
| `size` | `string` | `"1em"` | Valor de tamaño CSS aplicado al ancho y alto. |
| `color` | `string` | `"currentColor"` | Valor de color CSS. |
| `class` | `string` | — | Clases CSS adicionales. |

:::tip[Accesibilidad]
Agregue una prop `label` cuando un icono transmita significado (por ejemplo, un icono de enlace). Omita `label` para iconos puramente decorativos — Starlight los ocultará automáticamente de la tecnología de asistencia.
:::

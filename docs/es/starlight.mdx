...
Starlight incluye más de 200 iconos integrados disponibles a través del componente `<Icon>` — no se necesita instalación. El plugin `starlight-plugin-icons` ya está configurado en el tema, por lo que estos iconos funcionan en cualquier repositorio de contenido.
...
:::tip[Accesibilidad]
Añada una prop `label` cuando un icono transmita significado (por ejemplo, un icono de enlace). Omita `label` para iconos puramente decorativos — Starlight los ocultará automáticamente de la tecnología de asistencia.
:::
...
Los iconos Seti usan el prefijo `seti:` y están diseñados para representar tipos de archivo. Funcionan en el componente `<Icon>` y en el componente `<FileTree>`.

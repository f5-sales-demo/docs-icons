---
title: GCP アーキテクチャアイコン
description: >-
  GCP アーキテクチャアイコンのキュレーション済みリファレンス — クラウドインフラストラクチャの図やドキュメント向けに 216
  色のサービスアイコンを提供します。
sidebar:
  label: GCP
  order: 16
...
[GCP Architecture Icons](https://cloud.google.com/icons) は、Google Cloud インフラストラクチャのドキュメント作成に使用できる 216 色のサービスアイコンを提供します。`@f5-sales-demo/icons-gcp` の `Icon` コンポーネントを使用して、名前でアイコンをレンダリングできます。デフォルトサイズは 24x24 で、インラインフィルを使用します。
...
これらのアイコンはベンダーオリジナルの色（`currentColor` ではなく）を使用しています。色は SVG マークアップに埋め込まれており、ライトモードおよびダークモードの両方で保持されます。
...
216 種類の GCP アーキテクチャアイコンの完全なリストについては、[Google Cloud Architecture Icons](https://cloud.google.com/icons) ページをご覧ください。

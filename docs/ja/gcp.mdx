...
description: GCP アーキテクチャアイコンの厳選リファレンス — クラウドインフラストラクチャの図やドキュメント用の216個のカラーサービスアイコン。
sidebar:
  label: GCP
  order: 16
i18n:
...
これらのアイコンはベンダー独自のカラー（`currentColor` ではない）を使用します。色はSVGマークアップに埋め込まれており、ライトモードとダークモードの両方で保持されます。
...
216個のGCPアーキテクチャアイコンの完全なリストについては、[Google Cloud アーキテクチャアイコン](https://cloud.google.com/icons)のページをご覧ください。

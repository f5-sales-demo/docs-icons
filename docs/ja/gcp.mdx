...
description: GCP アーキテクチャアイコンの厳選リファレンス — クラウドインフラストラクチャ図やドキュメント用の216個のカラーサービスアイコン。
sidebar:
  label: GCP
  order: 16
i18n:
...
[GCP アーキテクチャアイコン](https://cloud.google.com/icons)は、Google Cloud インフラストラクチャのドキュメント作成用に216個のカラーサービスアイコンを提供します。`@f5xc-salesdemos/icons-gcp` の `Icon` コンポーネントを使用して、名前を指定して任意のアイコンをレンダリングできます。デフォルトサイズは24x24で、インラインフィルが適用されます。
...
これらのアイコンはベンダーのオリジナルカラーを使用しています（`currentColor` ではありません）。カラーはSVGマークアップに埋め込まれており、ライトモードとダークモードの両方で保持されます。
...
216個の GCP アーキテクチャアイコンの完全な一覧については、[Google Cloud アーキテクチャアイコン](https://cloud.google.com/icons)ページをご覧ください。

...
description: GCP アーキテクチャアイコンのキュレーションリファレンス — クラウドインフラストラクチャの図およびドキュメント用の 216 色のサービスアイコン。
sidebar:
  label: GCP
  order: 16
i18n:
...
[GCP Architecture Icons](https://cloud.google.com/icons) は、Google Cloud インフラストラクチャのドキュメント作成のために 216 色のサービスアイコンを提供しています。`@f5xc-salesdemos/icons-gcp` の `Icon` コンポーネントを使用して、名前で任意のアイコンをレンダリングします。デフォルトサイズはインラインフィルで 24x24 です。
...
これらのアイコンは元のベンダーカラー（`currentColor` ではない）を使用しています。カラーは SVG マークアップに埋め込まれており、ライトモードおよびダークモードの両方で保持されます。

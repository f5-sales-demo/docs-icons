...
description: GCP Architecture Icons の厳選リファレンス — クラウドインフラストラクチャ図やドキュメント用の216個のカラーサービスアイコン。
sidebar:
  label: GCP
  order: 16
i18n:
...
[GCP Architecture Icons](https://cloud.google.com/icons) は、Google Cloud インフラストラクチャのドキュメント作成用に216個のカラーサービスアイコンを提供しています。`@f5xc-salesdemos/icons-gcp` の `Icon` コンポーネントを使用して、名前を指定することで任意のアイコンをレンダリングできます。デフォルトサイズは 24x24 で、インラインフィルが適用されます。
...
これらのアイコンはベンダーのオリジナルカラーを使用しており（`currentColor` ではありません）。色は SVG マークアップに埋め込まれ、ライトモードとダークモードの両方で保持されます。
...
216個の GCP Architecture Icons の完全なリストについては、[Google Cloud Architecture Icons](https://cloud.google.com/icons) ページをご覧ください。

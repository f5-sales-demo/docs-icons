...
[Material Design Icons](https://icon-sets.iconify.design/mdi/) は、ほぼすべてのユースケースをカバーする 7,638 種類の塗りつぶしおよびアウトラインバリアントのアイコンを提供します。`@f5xc-salesdemos/icons-mdi` の `Icon` コンポーネントを使用して、名前を指定して任意のアイコンをレンダリングできます。
...
アイコンは `currentColor` を使用するインライン SVG としてレンダリングされるため、ライトモードとダークモードに自動的に適応します。
...
7,638 種類の Material Design Icons の完全なリストについては、[Iconify MDI エクスプローラー](https://icon-sets.iconify.design/mdi/)をご覧ください。

...
description: Azure アーキテクチャアイコンのキュレーションリファレンス — クラウドインフラダイアグラムおよびドキュメント用の606種類のカラーサービスアイコン。
sidebar:
  label: Azure
  order: 15
i18n:
...
[Azure Architecture Icons](https://learn.microsoft.com/en-us/azure/architecture/icons/) は、Azure インフラのドキュメント作成に使用できる606種類のカラーサービスアイコンを提供します。`@f5-sales-demo/icons-azure` の `Icon` コンポーネントを使用して、名前でアイコンをレンダリングします。デフォルトサイズはグラデーション塗りつぶしで18x18です。
...
これらのアイコンはオリジナルのベンダーカラー（`currentColor` ではない）を使用しています。カラーは SVG マークアップに埋め込まれており、ライトモードとダークモードの両方で保持されます。
...
606種類の Azure アーキテクチャアイコンの完全なリストについては、[Microsoft Azure Architecture Icons](https://learn.microsoft.com/en-us/azure/architecture/icons/) ページをご覧ください。

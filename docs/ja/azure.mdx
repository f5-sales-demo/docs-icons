---
title: Azure アーキテクチャ アイコン
description: Azure アーキテクチャ アイコンの厳選リファレンス — クラウドインフラストラクチャの図やドキュメント用の606個のカラーサービスアイコン。
sidebar:
  label: Azure
  order: 15
i18n:
...
[Azure Architecture Icons](https://learn.microsoft.com/en-us/azure/architecture/icons/) は、Azure インフラストラクチャのドキュメント作成用に606個のカラーサービスアイコンを提供しています。`@f5xc-salesdemos/icons-azure` の `Icon` コンポーネントを使用して、名前を指定して任意のアイコンをレンダリングできます。デフォルトサイズは18x18で、グラデーション塗りが適用されます。
...
これらのアイコンはベンダーのオリジナルカラーを使用しています（`currentColor` ではありません）。色は SVG マークアップに埋め込まれており、ライトモードとダークモードの両方で維持されます。
...
606個の Azure アーキテクチャ アイコンの完全なリストについては、[Microsoft Azure Architecture Icons](https://learn.microsoft.com/en-us/azure/architecture/icons/) ページをご覧ください。

...
description: Azure アーキテクチャアイコンのキュレーション済みリファレンス — クラウドインフラストラクチャ図およびドキュメント用の606色サービスアイコン。
sidebar:
  label: Azure
  order: 15
i18n:
...
[Azure Architecture Icons](https://learn.microsoft.com/en-us/azure/architecture/icons/) は、Azure インフラストラクチャをドキュメント化するための606色のサービスアイコンを提供します。`@f5xc-salesdemos/icons-azure` の `Icon` コンポーネントを使用して、名前で任意のアイコンをレンダリングします。デフォルトサイズはグラデーション塗りつぶしで18x18です。
...
これらのアイコンは元のベンダーカラー（`currentColor` ではない）を使用しています。色はSVGマークアップに埋め込まれており、ライトモードおよびダークモードの両方で保持されます。
...
606件のAzure アーキテクチャアイコンの完全なリストについては、[Microsoft Azure Architecture Icons](https://learn.microsoft.com/en-us/azure/architecture/icons/) ページをご覧ください。

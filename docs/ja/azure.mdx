...
description: >-
  Azure Architecture Icons の厳選リファレンス —
  クラウドインフラストラクチャの図やドキュメント用の606個のカラーサービスアイコン。
sidebar:
  label: Azure
  order: 15
i18n:
...
[Azure Architecture Icons](https://learn.microsoft.com/en-us/azure/architecture/icons/) は、Azure インフラストラクチャのドキュメント作成用に606個のカラーサービスアイコンを提供します。`@f5xc-salesdemos/icons-azure` の `Icon` コンポーネントを使用して、名前を指定することで任意のアイコンをレンダリングできます。デフォルトサイズは 18x18 で、グラデーション塗りが適用されます。
...
これらのアイコンはベンダーオリジナルのカラー（`currentColor` ではなく）を使用します。色は SVG マークアップに埋め込まれており、ライトモードとダークモードの両方で保持されます。
...
606個の Azure Architecture Icons の完全な一覧については、[Microsoft Azure Architecture Icons](https://learn.microsoft.com/en-us/azure/architecture/icons/) ページをご覧ください。

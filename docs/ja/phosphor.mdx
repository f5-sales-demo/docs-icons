...
[Phosphor Icons](https://icon-sets.iconify.design/ph/) は、regular、bold、duotone、fill、light、thin の柔軟なウェイトオプションを持つ 9,161 個のアイコンを提供します。`@f5xc-salesdemos/icons-phosphor` の `Icon` コンポーネントを使用して、名前で任意のアイコンをレンダリングできます。
...
:::note[ウェイトバリアント]
Phosphor アイコンには 6 つのウェイトがあります。regular ウェイトはベース名を使用します（例: `globe`）。他のウェイトにはサフィックスが付きます: `globe-bold`、`globe-duotone`、`globe-fill`、`globe-light`、`globe-thin`。以下の例は regular ウェイトを示しています。
:::
...
9,161 個の Phosphor Icons（すべてのウェイトバリアント）の完全なリストについては、[Iconify Phosphor エクスプローラー](https://icon-sets.iconify.design/ph/)をご覧ください。

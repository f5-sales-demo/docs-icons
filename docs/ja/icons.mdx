...
すべての `Icon` コンポーネントは `currentColor` を使用してインライン SVG をレンダリングします。現在のテーマに自動的に適応するため、特別な CSS やクラス処理は不要です。

F5 XC サービスアイコンは、ライトモードとダークモードに自動的に適応する CSS カスタムプロパティを使用しており、特別なクラス処理は不要です。その他のマルチカラーアイコン（HashiCorp Flight のカラーバリアントなど）については、`.icon-card-image` の `no-invert` クラスがダークモードで元の色を保持します。
...
| F5 製品ダイアグラム | F5 ブランド | ネットワーキング/セキュリティ分野に特化したアート |
...
| 柔軟なウェイトオプション | Phosphor | 6 つのウェイトバリアントを持つ 9,161 アイコン |
| シャープなラインアイコン | Tabler | 一貫した 2px ストロークの 6,034 アイコン |
| F5 XC サービスダイアグラム | F5 XC サービス | 30 の Distributed Cloud サービスアイコン |
| ブランド/企業ロゴ | Simple Icons | ベンダーやサービスの 3,200 以上のブランドアイコン |
| クラウド/インフラベンダーロゴ | HashiCorp Flight | AWS、GCP、Azure、K8s ベンダーアイコン |
| AWS アーキテクチャダイアグラム | AWS アーキテクチャ | AWS インフラ用の 885 カラーサービスアイコン |
| Azure アーキテクチャダイアグラム | Azure アーキテクチャ | Azure インフラ用の 606 カラーサービスアイコン |
| GCP アーキテクチャダイアグラム | GCP アーキテクチャ | Google Cloud インフラ用の 216 カラーサービスアイコン |

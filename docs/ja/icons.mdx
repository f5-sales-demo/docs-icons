...
ドキュメントテーマでは、複数のパックから数千のアイコンにアクセスできます。すべてのパックは同じパターンを使用します：パックの `Icon.astro` コンポーネントをインポートし、名前でアイコンを参照します。
...
| Simple Icons | 3,200以上 | `<Icon name="cloudflare" />` | [Simple Icons](/simple-icons/) |
...
すべてのアイコンパックは、パッケージごとの `Icon.astro` コンポーネントを使用してインライン SVG をレンダリングします。各パックには独自のインポートがあります：
...
F5 XC サービスアイコンは、ライトモードとダークモードに自動的に適応する CSS カスタムプロパティを使用しており、特別なクラスの処理は不要です。その他のマルチカラーアイコン（HashiCorp Flight のカラーバリアントなど）については、`.icon-card-image` の `no-invert` クラスにより、ダークモードでもオリジナルの色が保持されます。
...
| F5 製品ダイアグラム | F5 ブランド | ネットワーキング/セキュリティ専用のドメイン固有アート |
...
| ブランド/企業ロゴ | Simple Icons | ベンダーやサービス向けの 3,200 以上のブランドアイコン |
| クラウド/インフラベンダーロゴ | HashiCorp Flight | AWS、GCP、Azure、K8s ベンダーアイコン |
| AWS アーキテクチャダイアグラム | AWS アーキテクチャ | AWS インフラ向けの 885 のカラーサービスアイコン |
| Azure アーキテクチャダイアグラム | Azure アーキテクチャ | Azure インフラ向けの 606 のカラーサービスアイコン |
| GCP アーキテクチャダイアグラム | GCP アーキテクチャ | Google Cloud インフラ向けの 216 のカラーサービスアイコン |

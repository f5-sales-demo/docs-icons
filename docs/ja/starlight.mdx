---
title: Starlight 組み込みアイコン
description: Icon コンポーネントで利用可能な 200 以上の Starlight 組み込みアイコンの完全リファレンスです。
sidebar:
  label: Starlight 組み込み
  order: 3
i18n:
...
Starlight には `<Icon>` コンポーネントで利用可能な 200 以上の組み込みアイコンが含まれており、インストールは不要です。`starlight-plugin-icons` プラグインはテーマに既に設定されているため、これらのアイコンはどのコンテンツリポジトリでも動作します。

## Icon コンポーネントの使用

コンポーネントをインポートしてアイコン名を渡します：
...
| プロパティ | 型 | デフォルト | 説明 |
|------|------|---------|-------------|
| `name` | `string` | — | **必須。** 以下のテーブルに記載されているアイコン名。 |
| `label` | `string` | — | スクリーンリーダー向けのアクセシブルラベル。意味のあるアイコンに使用し、装飾的なアイコンでは省略します。 |
| `size` | `string` | `"1em"` | 幅と高さに適用される CSS サイズ値。 |
| `color` | `string` | `"currentColor"` | CSS カラー値。 |
| `class` | `string` | — | 追加の CSS クラス。 |

:::tip[アクセシビリティ]
アイコンが意味を伝える場合（例：リンクアイコン）は `label` プロパティを追加してください。純粋に装飾的なアイコンでは `label` を省略してください — Starlight が自動的に支援技術からアイコンを非表示にします。
:::
...
Seti アイコンは `seti:` プレフィックスを使用し、ファイルタイプの表現用に設計されています。`<Icon>` コンポーネントと `<FileTree>` コンポーネントの両方で使用できます。

---
title: Starlight ビルトインアイコン
description: Icon コンポーネントで利用可能な200以上のStarlightビルトインアイコンの完全なリファレンスです。
sidebar:
  label: Starlight ビルトイン
  order: 3
i18n:
...
Starlightには、`<Icon>` コンポーネントで利用可能な200以上のビルトインアイコンが含まれており、インストールは不要です。`starlight-plugin-icons` プラグインはテーマに既に設定されているため、これらのアイコンはどのコンテンツリポジトリでも動作します。
...
| プロパティ | 型 | デフォルト | 説明 |
|------|------|---------|-------------|
| `name` | `string` | — | **必須。** 以下の表に記載されたアイコン名。 |
| `label` | `string` | — | スクリーンリーダー用のアクセシブルラベル。意味を持つアイコンに使用し、装飾的なアイコンでは省略します。 |
| `size` | `string` | `"1em"` | 幅と高さに適用されるCSSサイズ値。 |
| `color` | `string` | `"currentColor"` | CSSカラー値。 |
| `class` | `string` | — | 追加のCSSクラス。 |

:::tip[アクセシビリティ]
アイコンが意味を伝える場合（例：リンクアイコン）は `label` プロパティを追加してください。純粋に装飾的なアイコンの場合は `label` を省略してください — Starlightは自動的に支援技術からそれらを非表示にします。
:::

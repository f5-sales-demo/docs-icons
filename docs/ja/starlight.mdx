...
description: Icon コンポーネントで利用可能な200以上の Starlight 組み込みアイコンの完全なリファレンスです。
sidebar:
...
Starlight には `<Icon>` コンポーネントで利用可能な200以上の組み込みアイコンが含まれており、インストールは不要です。`starlight-plugin-icons` プラグインはテーマに事前設定されているため、これらのアイコンはどのコンテンツリポジトリでも動作します。

## Icon コンポーネントの使用方法

コンポーネントをインポートし、アイコン名を渡します：
...
| `label` | `string` | — | スクリーンリーダー向けのアクセシブルラベル。意味のあるアイコンに使用し、装飾的なアイコンには省略してください。 |
| `size` | `string` | `"1em"` | 幅と高さに適用される CSS サイズ値。 |
| `color` | `string` | `"currentColor"` | CSS カラー値。 |
| `class` | `string` | — | 追加の CSS クラス。 |

:::tip[アクセシビリティ]
アイコンが意味を伝える場合は `label` prop を追加してください（例：リンクアイコン）。純粋に装飾的なアイコンの場合は `label` を省略してください。Starlight が自動的に支援技術から非表示にします。
:::
...
Seti アイコンは `seti:` プレフィックスを使用し、ファイルタイプの表現用に設計されています。`<Icon>` コンポーネントおよび `<FileTree>` コンポーネントで使用できます。

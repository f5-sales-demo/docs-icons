...
[AWS Architecture Icons](https://aws.amazon.com/architecture/icons/) は、AWSインフラストラクチャを文書化するための885色サービスアイコンを提供します。`@f5xc-salesdemos/icons-aws` の `Icon` コンポーネントを使用して、名前で任意のアイコンをレンダリングできます。デフォルトサイズは48x48で、アイコンごとにサイズを上書きできます。
...
これらのアイコンはベンダーオリジナルの色を使用します（`currentColor` ではありません）。色はSVGマークアップに埋め込まれており、ライトモードとダークモードの両方で保持されます。
...
885件のAWSアーキテクチャアイコンの完全なリストについては、[AWS Icons for Architecture Diagrams](https://aws.amazon.com/architecture/icons/) をご覧ください。

...
description: AWS アーキテクチャアイコンのキュレーションリファレンス — クラウドインフラストラクチャ図およびドキュメント用の885色サービスアイコン。
sidebar:
  label: AWS
  order: 14
i18n:
...
[AWS Architecture Icons](https://aws.amazon.com/architecture/icons/) は、AWSインフラストラクチャのドキュメント作成に使用できる885色のサービスアイコンを提供します。`@f5-sales-demo/icons-aws` の `Icon` コンポーネントを使用して、名前でアイコンをレンダリングします。デフォルトサイズは48x48で、アイコンごとにサイズを上書きできます。
...
これらのアイコンはベンダーオリジナルの色を使用しています（`currentColor` ではありません）。色はSVGマークアップに埋め込まれており、ライトモードおよびダークモードの両方で保持されます。
...
885個のAWSアーキテクチャアイコンの完全なリストについては、[AWS Icons for Architecture Diagrams](https://aws.amazon.com/architecture/icons/) をご覧ください。

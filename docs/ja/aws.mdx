...
description: AWS Architecture Icons の厳選リファレンス — クラウドインフラストラクチャの図やドキュメント用の885個のカラーサービスアイコン。
sidebar:
  label: AWS
  order: 14
i18n:
...
[AWS Architecture Icons](https://aws.amazon.com/architecture/icons/) は、AWSインフラストラクチャのドキュメント作成用に885個のカラーサービスアイコンを提供します。`@f5xc-salesdemos/icons-aws` の `Icon` コンポーネントを使用して、名前を指定して任意のアイコンをレンダリングできます。デフォルトサイズは48x48で、アイコンごとにサイズのオーバーライドが可能です。
...
これらのアイコンはベンダーオリジナルのカラー（`currentColor` ではありません）を使用しています。色はSVGマークアップに埋め込まれており、ライトモードとダークモードの両方で維持されます。
...
885個のAWS Architecture Iconsの完全なリストについては、[AWS Icons for Architecture Diagrams](https://aws.amazon.com/architecture/icons/) をご覧ください。

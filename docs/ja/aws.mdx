...
description: AWS アーキテクチャアイコンの厳選リファレンス — クラウドインフラストラクチャ図やドキュメント用の885個のカラーサービスアイコン。
sidebar:
  label: AWS
  order: 14
i18n:
...
[AWS Architecture Icons](https://aws.amazon.com/architecture/icons/) は、AWSインフラストラクチャのドキュメント作成用に885個のカラーサービスアイコンを提供します。`@f5xc-salesdemos/icons-aws` の `Icon` コンポーネントを使用して、名前を指定して任意のアイコンをレンダリングできます。デフォルトサイズは48x48で、アイコンごとのサイズオーバーライドが可能です。
...
これらのアイコンはベンダーオリジナルのカラーを使用しています（`currentColor` ではありません）。カラーはSVGマークアップに埋め込まれており、ライトモードとダークモードの両方で保持されます。

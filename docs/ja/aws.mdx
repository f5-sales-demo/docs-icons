...
description: AWS アーキテクチャアイコンの厳選リファレンス — クラウドインフラストラクチャの図やドキュメント用の885種類のカラーサービスアイコン。
sidebar:
  label: AWS
  order: 14
i18n:
...
[AWS Architecture Icons](https://aws.amazon.com/architecture/icons/) は、AWSインフラストラクチャのドキュメント作成用に885種類のカラーサービスアイコンを提供しています。`@f5xc-salesdemos/icons-aws` の `Icon` コンポーネントを使用して、名前を指定することで任意のアイコンをレンダリングできます。デフォルトサイズは48x48で、アイコンごとにサイズのオーバーライドが可能です。
...
これらのアイコンはベンダーオリジナルの色を使用しています（`currentColor` ではありません）。色はSVGマークアップに埋め込まれており、ライトモードとダークモードの両方で保持されます。
...
885種類のAWSアーキテクチャアイコンの完全なリストについては、[AWS Icons for Architecture Diagrams](https://aws.amazon.com/architecture/icons/) をご覧ください。

...
[`@f5xc-salesdemos/icons-hashicorp-flight`](https://www.npmjs.com/package/@f5xc-salesdemos/icons-hashicorp-flight) パッケージに含まれるすべてのアイコンの完全なビジュアルリファレンスです。アイコンは `Icon` コンポーネントを使用してレンダリングされ、カテゴリ別に整理されています。
...
アイコンは `currentColor` を使用したインライン SVG としてレンダリングされます — モノクロアイコンはライトモードとダークモードに自動的に対応します。カラーバリアントは元の色を保持します。

---
title: HashiCorp Flight アイコン
description: >-
  @f5-sales-demo/icons-hashicorp-flight の全672個の HashiCorp Flight
  アイコンのビジュアルリファレンスです。
sidebar:
...
[`@f5-sales-demo/icons-hashicorp-flight`](https://www.npmjs.com/package/@f5-sales-demo/icons-hashicorp-flight) パッケージに含まれるすべてのアイコンの完全なビジュアルリファレンスです。アイコンは `Icon` コンポーネントでレンダリングされ、カテゴリ別に整理されています。
...
アイコンは `currentColor` を使用したインラインSVGとしてレンダリングされます。モノクロアイコンはライトモードとダークモードに自動的に対応します。カラーバリアントは元の色を保持します。

:::note[ダークモード]
モノクロアイコンは `currentColor` を使用し、自動的に対応します。カラーバリアント（`-color` で終わるアイコン）は `no-invert` クラスにより元の色を保持します。
:::

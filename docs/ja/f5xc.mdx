...
title: F5 XC
description: >-
  @f5xc-salesdemos/icons-f5xc パッケージに含まれる全30個の F5 Distributed Cloud (XC)
  サービスアイコンのビジュアルリファレンスです。
sidebar:
  label: F5 XC
  order: 13
i18n:
...
[`@f5xc-salesdemos/icons-f5xc`](https://www.npmjs.com/package/@f5xc-salesdemos/icons-f5xc) パッケージに含まれるすべてのアイコンの完全なビジュアルリファレンスです。これらの30個のアイコンは F5 Distributed Cloud サービスを表しており、Astro コンポーネント付きの Iconify JSON として利用可能です。
...
または Iconify JSON を直接インポートすることもできます：
...
:::note[ダークモード]
これらのマルチカラーアイコンは CSS カスタムプロパティ（`--color-N600`、`--color-brand`、`--color-blue-light`、`--color-N200`）を使用しており、ライトモードとダークモードに自動的に適応します。ダークモードではネイビーとライトブルーが切り替わり、F5 の赤いアクセントカラーは一定のまま維持されます。
:::

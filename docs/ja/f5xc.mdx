...
[`@f5xc-salesdemos/icons-f5xc`](https://www.npmjs.com/package/@f5xc-salesdemos/icons-f5xc) パッケージに含まれるすべてのアイコンの完全なビジュアルリファレンスです。これら30個のアイコンは F5 Distributed Cloud サービスを表しており、Astro コンポーネント付きの Iconify JSON として利用可能です。
...
:::note[ダークモード]
これらのマルチカラーアイコンは CSS カスタムプロパティ（`--color-N600`、`--color-brand`、`--color-blue-light`、`--color-N200`）を使用しており、ライトモードとダークモードに自動的に適応します。ダークモードではネイビーとライトブルーが入れ替わりますが、F5 のレッドアクセントは常に一定です。
:::

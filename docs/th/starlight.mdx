---
title: ไอคอนในตัวของ Starlight
description: >-
  เอกสารอ้างอิงฉบับสมบูรณ์สำหรับไอคอนในตัวของ Starlight กว่า 200
  รายการที่ใช้งานได้ผ่านคอมโพเนนต์ Icon
sidebar:
  label: ไอคอนในตัวของ Starlight
  order: 3
i18n:
...
Starlight มีไอคอนในตัวมากกว่า 200 รายการที่ใช้งานได้ผ่านคอมโพเนนต์ `<Icon>` — ไม่ต้องติดตั้งเพิ่มเติม ปลั๊กอิน `starlight-plugin-icons` ได้ถูกกำหนดค่าไว้ในธีมแล้ว ดังนั้นไอคอนเหล่านี้จึงใช้งานได้ในทุก content repo

## การใช้คอมโพเนนต์ Icon

นำเข้าคอมโพเนนต์และส่งชื่อไอคอน:
...
| Prop | ชนิด | ค่าเริ่มต้น | คำอธิบาย |
|------|------|---------|-------------|
| `name` | `string` | — | **จำเป็น** ชื่อไอคอนจากตารางด้านล่าง |
| `label` | `string` | — | ป้ายกำกับสำหรับโปรแกรมอ่านหน้าจอ ใช้สำหรับไอคอนที่มีความหมาย ละเว้นสำหรับไอคอนที่เป็นการตกแต่ง |
| `size` | `string` | `"1em"` | ค่าขนาด CSS ที่ใช้กับความกว้างและความสูง |
| `color` | `string` | `"currentColor"` | ค่าสี CSS |
| `class` | `string` | — | คลาส CSS เพิ่มเติม |

:::tip[การเข้าถึง]
เพิ่ม prop `label` เมื่อไอคอนสื่อความหมาย (เช่น ไอคอนลิงก์) ละเว้น `label` สำหรับไอคอนที่เป็นการตกแต่งเท่านั้น — Starlight จะซ่อนไอคอนเหล่านั้นจากเทคโนโลยีช่วยเหลือโดยอัตโนมัติ
:::
...
ไอคอน Seti ใช้คำนำหน้า `seti:` และได้รับการออกแบบสำหรับการแสดงประเภทไฟล์ ไอคอนเหล่านี้ใช้งานได้ทั้งในคอมโพเนนต์ `<Icon>` และคอมโพเนนต์ `<FileTree>`

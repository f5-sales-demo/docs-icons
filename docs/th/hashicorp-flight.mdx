...
description: >-
  ข้อมูลอ้างอิงภาพสำหรับไอคอน HashiCorp Flight ทั้งหมด 672 รายการจาก
...
ข้อมูลอ้างอิงภาพที่ครบถ้วนสำหรับไอคอนทุกตัวในแพ็กเกจ [`@f5xc-salesdemos/icons-hashicorp-flight`](https://www.npmjs.com/package/@f5xc-salesdemos/icons-hashicorp-flight) ไอคอนถูกแสดงผลผ่านคอมโพเนนต์ `Icon` และจัดหมวดหมู่ตามประเภท
...
ไอคอนจะแสดงผลเป็น SVG แบบ inline โดยใช้ `currentColor` — ไอคอนสีเดียวจะปรับตัวตามโหมดสว่างและโหมดมืดโดยอัตโนมัติ ส่วนตัวแปรสีจะคงสีเดิมไว้

:::note[โหมดมืด]
ไอคอนสีเดียวใช้ `currentColor` และปรับตัวโดยอัตโนมัติ ตัวแปรสี (ไอคอนที่ลงท้ายด้วย `-color`) จะคงสีเดิมไว้ผ่านคลาส `no-invert`
:::

---
title: Icônes d'architecture GCP
description: >-
  Référence organisée pour les icônes d'architecture GCP — 216 icônes de service
...
[GCP Architecture Icons](https://cloud.google.com/icons) fournit 216 icônes de service en couleur pour documenter l'infrastructure Google Cloud. Utilisez le composant `Icon` de `@f5-sales-demo/icons-gcp` pour afficher n'importe quelle icône par son nom. La taille par défaut est 24x24 avec des remplissages intégrés.
...
Ces icônes utilisent les couleurs d'origine du fournisseur (et non `currentColor`). Les couleurs sont intégrées dans le balisage SVG et sont préservées en mode clair comme en mode sombre.
...
Pour la liste complète des 216 icônes d'architecture GCP, visitez la page [Google Cloud Architecture Icons](https://cloud.google.com/icons).

...
[GCP Architecture Icons](https://cloud.google.com/icons) fournit 216 icônes de services en couleur pour documenter l'infrastructure Google Cloud. Utilisez le composant `Icon` de `@f5xc-salesdemos/icons-gcp` pour afficher n'importe quelle icône par son nom. La taille par défaut est 24x24 avec des remplissages en ligne.
...
Ces icônes utilisent les couleurs originales du fournisseur (pas `currentColor`). Les couleurs sont intégrées dans le balisage SVG et préservées en mode clair comme en mode sombre.

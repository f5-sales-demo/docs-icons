...
description: >-
  Vue d'ensemble de tous les packs d'icônes disponibles dans le thème de
  documentation — utilisation, comportement en mode sombre et guide de
  sélection.
sidebar:
...
Le thème de documentation donne accès à des milliers d'icônes provenant de plusieurs packs. Chaque pack utilise le même modèle : importez le composant `Icon.astro` du pack et référencez les icônes par leur nom.
...
Tous les packs d'icônes utilisent des composants `Icon.astro` par paquet qui effectuent le rendu de SVG en ligne. Chaque pack possède sa propre importation :
...
Les icônes utilisent `currentColor` et s'adaptent automatiquement aux modes clair et sombre sans aucun traitement spécial nécessaire.
...
## Comportement mode sombre/clair

Tous les composants `Icon` effectuent le rendu de SVG en ligne utilisant `currentColor`. Ils s'adaptent automatiquement au thème actuel — aucun traitement CSS ou de classe spécial n'est nécessaire.
...
|-------------------|-----------------|----------|
| Interface utilisateur générale (flèches, fermeture, recherche) | Starlight intégré | Aucune configuration, toujours disponible |
...
| Icônes pleines/contourées à grande échelle | Material Design (mdi) | Le plus grand ensemble avec 7 638 icônes |
...
| Icônes en ligne nettes | Tabler | 6 034 icônes avec un trait cohérent de 2px |
...
| Logos de fournisseurs cloud/infrastructure | HashiCorp Flight | Icônes de fournisseurs AWS, GCP, Azure, K8s |

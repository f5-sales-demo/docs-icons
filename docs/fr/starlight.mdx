...
| `size` | `string` | `"1em"` | Valeur de taille CSS appliquée à la largeur et la hauteur. |
| `color` | `string` | `"currentColor"` | Valeur de couleur CSS. |
| `class` | `string` | — | Classes CSS supplémentaires. |

:::tip[Accessibilité]
Ajoutez une propriété `label` lorsqu'une icône véhicule un sens (par ex. une icône de lien). Omettez `label` pour les icônes purement décoratives — Starlight les masquera automatiquement des technologies d'assistance.
:::
...
Les icônes Seti utilisent le préfixe `seti:` et sont conçues pour représenter les types de fichiers. Elles fonctionnent dans le composant `<Icon>` et le composant `<FileTree>`.

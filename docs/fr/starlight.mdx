---
title: Icônes intégrées de Starlight
description: >-
  Référence complète de toutes les 200+ icônes intégrées de Starlight
  disponibles via le composant Icon.
sidebar:
  label: Icônes intégrées de Starlight
  order: 3
i18n:
...
Starlight inclut plus de 200 icônes intégrées disponibles via le composant `<Icon>` — aucune installation nécessaire. Le plugin `starlight-plugin-icons` est déjà configuré dans le thème, ces icônes fonctionnent donc dans n'importe quel dépôt de contenu.

## Utilisation du composant Icon

Importez le composant et passez un nom d'icône :
...
|------|------|---------|-------------|
| `name` | `string` | — | **Obligatoire.** Le nom de l'icône parmi les tableaux ci-dessous. |
| `label` | `string` | — | Libellé accessible pour les lecteurs d'écran. À utiliser pour les icônes significatives ; à omettre pour les icônes décoratives. |
| `size` | `string` | `"1em"` | Valeur de taille CSS appliquée à la largeur et à la hauteur. |
| `color` | `string` | `"currentColor"` | Valeur de couleur CSS. |
| `class` | `string` | — | Classes CSS supplémentaires. |

:::tip[Accessibilité]
Ajoutez une propriété `label` lorsqu'une icône transmet un sens (par ex. une icône de lien). Omettez `label` pour les icônes purement décoratives — Starlight les masquera automatiquement des technologies d'assistance.
:::
...
## Icônes sociales et communautaires
...
## Icônes de technologies et frameworks
...
## Icônes de fichiers Seti

Les icônes Seti utilisent le préfixe `seti:` et sont conçues pour la représentation des types de fichiers. Elles fonctionnent dans le composant `<Icon>` et dans le composant `<FileTree>`.

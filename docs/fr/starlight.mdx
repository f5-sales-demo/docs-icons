...
description: >-
  Référence complète des plus de 200 icônes intégrées de Starlight disponibles
  via le composant Icon.
sidebar:
...
|------|------|---------|-------------|
| `name` | `string` | — | **Requis.** Le nom de l'icône parmi les tableaux ci-dessous. |
| `label` | `string` | — | Libellé accessible pour les lecteurs d'écran. À utiliser pour les icônes porteuses de sens ; à omettre pour les icônes décoratives. |
| `size` | `string` | `"1em"` | Valeur CSS de taille appliquée à la largeur et la hauteur. |
| `color` | `string` | `"currentColor"` | Valeur CSS de couleur. |
| `class` | `string` | — | Classes CSS supplémentaires. |

:::tip[Accessibilité]
Ajoutez une propriété `label` lorsqu'une icône a une signification (par exemple, une icône de lien). Omettez `label` pour les icônes purement décoratives — Starlight les masquera automatiquement des technologies d'assistance.
:::
...
## Icônes de réseaux sociaux et communauté

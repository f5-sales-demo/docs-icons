---
title: HashiCorp Flight 아이콘
description: >-
  @f5-sales-demo/icons-hashicorp-flight의 672개 HashiCorp Flight 아이콘 전체 시각적
  참조입니다.
sidebar:
...
[`@f5-sales-demo/icons-hashicorp-flight`](https://www.npmjs.com/package/@f5-sales-demo/icons-hashicorp-flight) 패키지의 모든 아이콘에 대한 전체 시각적 참조입니다. 아이콘은 `Icon` 컴포넌트를 통해 렌더링되며 카테고리별로 정리되어 있습니다.
...
아이콘은 `currentColor`를 사용하는 인라인 SVG로 렌더링됩니다 — 단색 아이콘은 라이트 모드와 다크 모드에 자동으로 적응합니다. 컬러 변형은 원래 색상을 유지합니다.

:::note[다크 모드]
단색 아이콘은 `currentColor`를 사용하며 자동으로 적응합니다. 컬러 변형(`-color`로 끝나는 아이콘)은 `no-invert` 클래스를 통해 원래 색상을 유지합니다.
:::

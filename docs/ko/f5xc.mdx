...
title: F5 XC
description: >-
  @f5xc-salesdemos/icons-f5xc 패키지의 30개 F5 Distributed Cloud (XC) 서비스 아이콘에 대한 시각적
  참조입니다.
sidebar:
  label: F5 XC
  order: 13
...
[`@f5xc-salesdemos/icons-f5xc`](https://www.npmjs.com/package/@f5xc-salesdemos/icons-f5xc) 패키지에 포함된 모든 아이콘의 전체 시각적 참조입니다. 이 30개의 아이콘은 F5 Distributed Cloud 서비스를 나타내며, Astro 컴포넌트와 함께 Iconify JSON으로 제공됩니다.
...
:::note[다크 모드]
이 다색 아이콘들은 라이트 모드와 다크 모드에 자동으로 적응하는 CSS 커스텀 속성(`--color-N600`, `--color-brand`, `--color-blue-light`, `--color-N200`)을 사용합니다. 다크 모드에서는 네이비와 라이트 블루가 서로 전환되며, F5 레드 액센트는 일정하게 유지됩니다.
:::

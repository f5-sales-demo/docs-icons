---
title: GCP 아키텍처 아이콘
description: GCP 아키텍처 아이콘에 대한 정리된 참조 문서 — 클라우드 인프라 다이어그램 및 문서화를 위한 216개의 컬러 서비스 아이콘.
sidebar:
  label: GCP
  order: 16
i18n:
...
[GCP 아키텍처 아이콘](https://cloud.google.com/icons)은 Google Cloud 인프라 문서화를 위한 216개의 컬러 서비스 아이콘을 제공합니다. `@f5xc-salesdemos/icons-gcp`의 `Icon` 컴포넌트를 사용하여 이름으로 아이콘을 렌더링할 수 있습니다. 기본 크기는 24x24이며 인라인 채우기를 사용합니다.
...
이 아이콘들은 원본 벤더 색상을 사용합니다(`currentColor`가 아님). 색상은 SVG 마크업에 내장되어 있으며 라이트 모드와 다크 모드 모두에서 유지됩니다.

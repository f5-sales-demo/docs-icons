---
title: GCP 아키텍처 아이콘
description: GCP 아키텍처 아이콘에 대한 선별된 참조 — 클라우드 인프라 다이어그램 및 문서용 216개의 컬러 서비스 아이콘.
sidebar:
  label: GCP
  order: 16
i18n:
...
[GCP 아키텍처 아이콘](https://cloud.google.com/icons)은 Google Cloud 인프라 문서화를 위한 216개의 컬러 서비스 아이콘을 제공합니다. `@f5-sales-demo/icons-gcp`의 `Icon` 구성 요소를 사용하여 이름으로 아이콘을 렌더링하세요. 기본 크기는 인라인 채우기를 포함한 24x24입니다.
...
이 아이콘들은 `currentColor` 대신 원본 벤더 색상을 사용합니다. 색상은 SVG 마크업에 내장되어 있으며 라이트 모드와 다크 모드 모두에서 유지됩니다.
...
216개의 GCP 아키텍처 아이콘 전체 목록은 [Google Cloud 아키텍처 아이콘](https://cloud.google.com/icons) 페이지를 방문하세요.

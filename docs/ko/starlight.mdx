---
title: Starlight 내장 아이콘
description: Icon 컴포넌트를 통해 사용할 수 있는 200개 이상의 Starlight 내장 아이콘에 대한 전체 참조 문서입니다.
sidebar:
  label: Starlight 내장 아이콘
  order: 3
i18n:
...
:::tip[접근성]
아이콘이 의미를 전달하는 경우(예: 링크 아이콘) `label` prop을 추가하세요. 순수하게 장식용인 아이콘에는 `label`을 생략하세요 — Starlight가 자동으로 보조 기술에서 해당 아이콘을 숨깁니다.
:::
...
Seti 아이콘은 `seti:` 접두사를 사용하며 파일 유형을 나타내기 위해 설계되었습니다. `<Icon>` 컴포넌트와 `<FileTree>` 컴포넌트에서 모두 사용할 수 있습니다.

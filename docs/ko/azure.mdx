...
description: >-
  Azure Architecture Icons에 대한 엄선된 레퍼런스 — 클라우드 인프라 다이어그램 및 문서화를 위한 606개의 컬러 서비스
  아이콘.
sidebar:
  label: Azure
  order: 15
...
[Azure Architecture Icons](https://learn.microsoft.com/en-us/azure/architecture/icons/)는 Azure 인프라를 문서화하기 위한 606개의 컬러 서비스 아이콘을 제공합니다. `@f5xc-salesdemos/icons-azure`의 `Icon` 컴포넌트를 사용하여 이름으로 원하는 아이콘을 렌더링할 수 있습니다. 기본 크기는 18x18이며 그라디언트 채우기를 사용합니다.
...
이 아이콘들은 원래 벤더 색상을 사용합니다(`currentColor`가 아님). 색상은 SVG 마크업에 내장되어 있으며 라이트 모드와 다크 모드 모두에서 유지됩니다.
...
606개의 Azure Architecture Icons 전체 목록은 [Microsoft Azure Architecture Icons](https://learn.microsoft.com/en-us/azure/architecture/icons/) 페이지를 방문하세요.

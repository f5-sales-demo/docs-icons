...
description: Azure 아키텍처 아이콘에 대한 큐레이션 참조 — 클라우드 인프라 다이어그램 및 문서화를 위한 606개의 컬러 서비스 아이콘.
sidebar:
  label: Azure
  order: 15
i18n:
...
[Azure 아키텍처 아이콘](https://learn.microsoft.com/en-us/azure/architecture/icons/)은 Azure 인프라 문서화를 위한 606개의 컬러 서비스 아이콘을 제공합니다. `@f5xc-salesdemos/icons-azure`의 `Icon` 구성 요소를 사용하여 이름으로 아이콘을 렌더링하세요. 기본 크기는 그라디언트 채우기로 18x18입니다.
...
이 아이콘들은 `currentColor` 대신 원본 벤더 색상을 사용합니다. 색상은 SVG 마크업에 내장되어 라이트 모드와 다크 모드 모두에서 보존됩니다.

...
description: 'Phosphor Icons의 선별된 참조 — Icon 구성 요소를 통해 유연한 두께 변형을 제공하는 9,161개의 아이콘.'
sidebar:
  label: Phosphor
  order: 8
i18n:
...
[Phosphor Icons](https://icon-sets.iconify.design/ph/)는 regular, bold, duotone, fill, light, thin 등 유연한 두께 옵션을 갖춘 9,161개의 아이콘을 제공합니다. `@f5xc-salesdemos/icons-phosphor`의 `Icon` 구성 요소를 사용하여 이름으로 아이콘을 렌더링하세요.
...
아이콘은 `currentColor`를 사용하는 인라인 SVG로 렌더링됩니다 — 라이트 모드와 다크 모드에 자동으로 적응합니다.

:::note[두께 변형]
Phosphor 아이콘은 여섯 가지 두께로 제공됩니다. regular 두께는 기본 이름을 사용합니다 (예: `globe`). 다른 두께는 접미사를 추가합니다: `globe-bold`, `globe-duotone`, `globe-fill`, `globe-light`, `globe-thin`. 아래 예시는 regular 두께를 보여줍니다.
:::
...
9,161개의 전체 Phosphor Icons 목록(모든 두께 변형 포함)은 [Iconify Phosphor 탐색기](https://icon-sets.iconify.design/ph/)를 방문하세요.

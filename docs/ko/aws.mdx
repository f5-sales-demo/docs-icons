---
title: AWS 아키텍처 아이콘
description: AWS 아키텍처 아이콘에 대한 선별된 참조 — 클라우드 인프라 다이어그램 및 문서화를 위한 885개의 컬러 서비스 아이콘.
sidebar:
  label: AWS
  order: 14
i18n:
...
[AWS 아키텍처 아이콘](https://aws.amazon.com/architecture/icons/)은 AWS 인프라 문서화를 위한 885개의 컬러 서비스 아이콘을 제공합니다. `@f5-sales-demo/icons-aws`의 `Icon` 구성 요소를 사용하여 이름으로 모든 아이콘을 렌더링하세요. 기본 크기는 48x48이며 아이콘별 크기 재정의가 가능합니다.
...
이 아이콘들은 `currentColor` 대신 원래 벤더 색상을 사용합니다. 색상은 SVG 마크업에 내장되어 있으며 라이트 모드와 다크 모드 모두에서 유지됩니다.
...
885개의 AWS 아키텍처 아이콘 전체 목록은 [AWS 아키텍처 다이어그램용 아이콘](https://aws.amazon.com/architecture/icons/)을 방문하세요.

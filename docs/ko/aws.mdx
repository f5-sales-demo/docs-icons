---
title: AWS 아키텍처 아이콘
description: AWS 아키텍처 아이콘에 대한 큐레이션된 참조 — 클라우드 인프라 다이어그램 및 문서를 위한 885개의 컬러 서비스 아이콘.
sidebar:
  label: AWS
  order: 14
i18n:
...
이 아이콘들은 원래 벤더 색상을 사용합니다(`currentColor`가 아님). 색상은 SVG 마크업에 포함되어 있으며 라이트 모드와 다크 모드 모두에서 유지됩니다.

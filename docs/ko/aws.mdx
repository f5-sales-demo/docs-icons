...
description: >-
  AWS Architecture Icons에 대한 정리된 참조 자료 — 클라우드 인프라 다이어그램 및 문서화를 위한 885개의 컬러 서비스
  아이콘.
sidebar:
  label: AWS
  order: 14
...
[AWS Architecture Icons](https://aws.amazon.com/architecture/icons/)는 AWS 인프라 문서화를 위한 885개의 컬러 서비스 아이콘을 제공합니다. `@f5xc-salesdemos/icons-aws`의 `Icon` 컴포넌트를 사용하여 이름으로 아이콘을 렌더링할 수 있습니다. 기본 크기는 48x48이며, 아이콘별로 크기를 재정의할 수 있습니다.
...
이 아이콘들은 원본 벤더 색상을 사용합니다(`currentColor`가 아님). 색상은 SVG 마크업에 포함되어 있으며, 라이트 모드와 다크 모드 모두에서 유지됩니다.
...
885개의 AWS Architecture Icons 전체 목록을 확인하려면 [AWS Icons for Architecture Diagrams](https://aws.amazon.com/architecture/icons/)를 방문하세요.

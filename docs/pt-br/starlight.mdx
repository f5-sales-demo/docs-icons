...
O Starlight inclui mais de 200 ícones integrados disponíveis via o componente `<Icon>` — sem necessidade de instalação. O plugin `starlight-plugin-icons` já está configurado no tema, então esses ícones funcionam em qualquer repositório de conteúdo.
...
|------|------|--------|-----------|
| `name` | `string` | — | **Obrigatória.** O nome do ícone das tabelas abaixo. |
| `label` | `string` | — | Rótulo acessível para leitores de tela. Use para ícones significativos; omita para ícones decorativos. |
...
:::tip[Acessibilidade]
Adicione a prop `label` quando um ícone transmite significado (por exemplo, um ícone de link). Omita `label` para ícones puramente decorativos — o Starlight os ocultará automaticamente de tecnologias assistivas.
:::
...
## Ícones de Tecnologia &amp; Frameworks

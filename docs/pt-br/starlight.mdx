---
title: Ícones Integrados do Starlight
description: >-
  Referência completa para todos os mais de 200 ícones integrados do Starlight
  disponíveis através do componente Icon.
sidebar:
  label: Integrados do Starlight
  order: 3
i18n:
...
O Starlight inclui mais de 200 ícones integrados disponíveis através do componente `<Icon>` — sem necessidade de instalação. O plugin `starlight-plugin-icons` já está configurado no tema, então esses ícones funcionam em qualquer repositório de conteúdo.

## Usando o Componente Icon

Importe o componente e passe um nome de ícone:
...
|------|------|---------|-------------|
| `name` | `string` | — | **Obrigatório.** O nome do ícone das tabelas abaixo. |
| `label` | `string` | — | Rótulo acessível para leitores de tela. Use para ícones com significado; omita para ícones decorativos. |
| `size` | `string` | `"1em"` | Valor de tamanho CSS aplicado à largura e altura. |
| `color` | `string` | `"currentColor"` | Valor de cor CSS. |
| `class` | `string` | — | Classes CSS adicionais. |

:::tip[Acessibilidade]
Adicione uma propriedade `label` quando um ícone transmite significado (por exemplo, um ícone de link). Omita `label` para ícones puramente decorativos — o Starlight os ocultará automaticamente das tecnologias assistivas.
:::
...
## Ícones de Tecnologia &amp; Framework
...
## Ícones de Arquivo Seti

Os ícones Seti usam o prefixo `seti:` e são projetados para representação de tipos de arquivo. Eles funcionam no componente `<Icon>` e no componente `<FileTree>`.

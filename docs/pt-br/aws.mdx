...
description: >-
  Referência selecionada para Ícones de Arquitetura AWS — 885 ícones de serviços
  coloridos para diagramas de infraestrutura em nuvem e documentação.
sidebar:
  label: AWS
  order: 14
i18n:
...
[AWS Architecture Icons](https://aws.amazon.com/architecture/icons/) fornece 885 ícones de serviços coloridos para documentar infraestruturas AWS. Utilize o componente `Icon` de `@f5xc-salesdemos/icons-aws` para renderizar qualquer ícone pelo nome. O tamanho padrão é 48x48 com substituições de tamanho por ícone.
...
Esses ícones utilizam as cores originais do fornecedor (não `currentColor`). As cores estão incorporadas na marcação SVG e são preservadas nos modos claro e escuro.

...
[AWS Architecture Icons](https://aws.amazon.com/architecture/icons/) fornece 885 ícones coloridos de serviços para documentar infraestrutura AWS. Use o componente `Icon` de `@f5xc-salesdemos/icons-aws` para renderizar qualquer ícone pelo nome. O tamanho padrão é 48x48 com substituições de tamanho por ícone.
...
Esses ícones usam as cores originais do fornecedor (não `currentColor`). As cores são incorporadas na marcação SVG e preservadas tanto no modo claro quanto no modo escuro.

...
O tema de documentação fornece acesso a milhares de ícones de múltiplos pacotes. Todos os pacotes utilizam o mesmo padrão: importe o componente `Icon.astro` do pacote e referencie os ícones pelo nome.
...
Todos os pacotes de ícones utilizam componentes `Icon.astro` por pacote que renderizam SVGs inline. Cada pacote tem sua própria importação:
...
Todos os componentes `Icon` renderizam SVGs inline utilizando `currentColor`. Eles se adaptam automaticamente ao tema atual — nenhum tratamento especial de CSS ou classes é necessário.

Os ícones de serviços F5 XC utilizam propriedades customizadas CSS que se adaptam automaticamente aos modos claro e escuro — nenhum tratamento especial de classes é necessário. Para outros ícones multicoloridos (como variantes coloridas do HashiCorp Flight), a classe `no-invert` em `.icon-card-image` preserva as cores originais no modo escuro.

## Qual Pacote Devo Usar?

| Caso de Uso | Pacote Recomendado | Por quê |
|-------------|-------------------|---------|
| UI geral (setas, fechar, buscar) | Starlight Integrado | Sem configuração, sempre disponível |
| Diagramas de produtos F5 | F5 Brand | Arte específica de domínio para redes/segurança |
...
| Opções flexíveis de peso | Phosphor | 9.161 ícones com seis variantes de peso |
...
| Logos de fornecedores cloud/infra | HashiCorp Flight | Ícones de fornecedores AWS, GCP, Azure, K8s |

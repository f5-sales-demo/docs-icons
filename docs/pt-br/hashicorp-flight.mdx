...
description: >-
  Referência visual de todos os 672 ícones HashiCorp Flight do pacote
...
Referência visual completa de todos os ícones do pacote [`@f5-sales-demo/icons-hashicorp-flight`](https://www.npmjs.com/package/@f5-sales-demo/icons-hashicorp-flight). Os ícones são renderizados pelo componente `Icon` e organizados por categoria.
...
Os ícones são renderizados como SVGs inline usando `currentColor` — os ícones monocromáticos se adaptam automaticamente aos modos claro e escuro. As variantes coloridas preservam suas cores originais.

:::note[Modo Escuro]
Os ícones monocromáticos utilizam `currentColor` e se adaptam automaticamente. As variantes coloridas (ícones com sufixo `-color`) preservam suas cores originais por meio da classe `no-invert`.
:::

...
description: >-
  Referência visual para todos os 672 ícones HashiCorp Flight do
...
Referência visual completa para cada ícone no pacote [`@f5xc-salesdemos/icons-hashicorp-flight`](https://www.npmjs.com/package/@f5xc-salesdemos/icons-hashicorp-flight). Os ícones são renderizados através do componente `Icon` e organizados por categoria.
...
:::note[Modo Escuro]
Ícones monocromáticos usam `currentColor` e se adaptam automaticamente. Variantes coloridas (ícones terminados em `-color`) preservam suas cores originais através da classe `no-invert`.
:::

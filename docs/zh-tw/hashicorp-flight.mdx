...
description: 來自 @f5xc-salesdemos/icons-hashicorp-flight 的全部 672 個 HashiCorp Flight 圖示視覺參考。
sidebar:
...
圖示以內聯 SVG 方式渲染，使用 `currentColor`——單色圖示可自動適應明暗模式。彩色變體保留其原始顏色。

:::note[深色模式]
單色圖示使用 `currentColor`，可自動適應。彩色變體（圖示名稱以 `-color` 結尾）透過 `no-invert` class 保留其原始顏色。
:::

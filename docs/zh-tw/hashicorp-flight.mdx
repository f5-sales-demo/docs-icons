---
title: HashiCorp Flight 圖示
description: 來自 @f5-sales-demo/icons-hashicorp-flight 的所有 672 個 HashiCorp Flight 圖示視覺參考。
sidebar:
...
[`@f5-sales-demo/icons-hashicorp-flight`](https://www.npmjs.com/package/@f5-sales-demo/icons-hashicorp-flight) 套件中每個圖示的完整視覺參考。圖示透過 `Icon` 元件渲染，並依類別分組。
...
圖示以內嵌 SVG 的形式使用 `currentColor` 渲染 — 單色圖示會自動適應淺色與深色模式。彩色變體則保留其原始色彩。

:::note[深色模式]
單色圖示使用 `currentColor`，會自動適應。彩色變體（名稱以 `-color` 結尾的圖示）透過 `no-invert` 類別保留其原始色彩。
:::

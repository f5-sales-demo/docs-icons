...
[`@f5xc-salesdemos/icons-hashicorp-flight`](https://www.npmjs.com/package/@f5xc-salesdemos/icons-hashicorp-flight) 套件中每個圖示的完整視覺參考。圖示透過 `Icon` 元件渲染，並按類別分類。
...
圖示以內嵌 SVG 方式渲染，使用 `currentColor` — 單色圖示會自動適應淺色和深色模式。彩色變體則保留其原始顏色。

:::note[深色模式]
單色圖示使用 `currentColor` 並自動適應。彩色變體（以 `-color` 結尾的圖示）透過 `no-invert` 類別保留其原始顏色。
:::

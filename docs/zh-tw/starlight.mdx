---
title: Starlight 內建圖示
description: 透過 Icon 元件使用超過 200 個以上 Starlight 內建圖示的完整參考。
sidebar:
  label: Starlight 內建圖示
  order: 3
i18n:
...
Starlight 提供超過 200 個內建圖示，可透過 `<Icon>` 元件使用——無需額外安裝。主題中已預先設定 `starlight-plugin-icons` 外掛，因此這些圖示可在任何內容儲存庫中使用。
...
| `label` | `string` | — | 供螢幕閱讀器使用的無障礙標籤。用於有含義的圖示；裝飾性圖示可省略。 |
| `size` | `string` | `"1em"` | 套用於寬度和高度的 CSS 尺寸值。 |
| `color` | `string` | `"currentColor"` | CSS 顏色值。 |
| `class` | `string` | — | 額外的 CSS 類別。 |

:::tip[無障礙性]
當圖示傳達含義時（例如連結圖示），請加入 `label` 屬性。純裝飾性圖示可省略 `label`——Starlight 會自動將其從輔助技術中隱藏。
:::
...
Seti 圖示使用 `seti:` 前綴，專為檔案類型表示而設計。它們可在 `<Icon>` 元件和 `<FileTree>` 元件中使用。

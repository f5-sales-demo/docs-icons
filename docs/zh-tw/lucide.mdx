---
title: Lucide 圖示
description: 'Lucide 圖示的精選參考資料 — 透過 Icon 元件可使用 1,600 多個現代描邊圖示。'
sidebar:
  label: Lucide
  order: 5
i18n:
...
[Lucide](https://lucide.dev/) 提供約 1,600 個現代、一致的描邊圖示。使用來自 `@f5xc-salesdemos/icons-lucide` 的 `Icon` 元件，即可依名稱渲染任意圖示。
...
圖示以內嵌 SVG 的形式渲染，使用 `currentColor` — 可自動適應亮色與暗色模式。
...
如需完整的約 1,600 個 Lucide 圖示清單，請前往 [lucide.dev/icons](https://lucide.dev/icons/)。

---
title: Lucide 圖示
description: 'Lucide 圖示精選參考手冊 — 透過 Icon 元件提供 1,600 多個現代筆觸圖示。'
sidebar:
  label: Lucide
  order: 5
i18n:
...
[Lucide](https://lucide.dev/) 提供約 1,600 個現代、一致的筆觸圖示。使用來自 `@f5xc-salesdemos/icons-lucide` 的 `Icon` 元件，以名稱渲染任意圖示。
...
圖示以內嵌 SVG 的形式渲染，使用 `currentColor` — 可自動適應明亮與暗黑模式。
...
如需完整的約 1,600 個 Lucide 圖示清單，請造訪 [lucide.dev/icons](https://lucide.dev/icons/)。

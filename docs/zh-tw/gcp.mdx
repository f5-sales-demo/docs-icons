...
[GCP 架構圖示](https://cloud.google.com/icons) 提供 216 個彩色服務圖示，用於記錄 Google Cloud 基礎架構。使用 `@f5xc-salesdemos/icons-gcp` 中的 `Icon` 元件，透過名稱渲染任何圖示。預設大小為 24x24，並使用內嵌填色。
...
這些圖示使用原始供應商顏色（非 `currentColor`）。顏色嵌入於 SVG 標記中，並在淺色和深色模式下均予以保留。

---
title: GCP 架構圖示
description: GCP 架構圖示精選參考 — 216 個彩色服務圖示，適用於雲端基礎架構圖表與文件。
sidebar:
  label: GCP
  order: 16
i18n:
...
[GCP 架構圖示](https://cloud.google.com/icons) 提供 216 個彩色服務圖示，用於記錄 Google Cloud 基礎架構。使用 `@f5xc-salesdemos/icons-gcp` 中的 `Icon` 元件，即可透過名稱渲染任何圖示。預設大小為 24x24，並使用內嵌填色。
...
這些圖示使用原始供應商顏色（非 `currentColor`）。顏色嵌入在 SVG 標記中，在淺色與深色模式下均會保留。
...
如需查看完整的 216 個 GCP 架構圖示清單，請造訪 [Google Cloud 架構圖示](https://cloud.google.com/icons) 頁面。

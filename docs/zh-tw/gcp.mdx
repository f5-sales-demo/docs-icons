---
title: GCP 架構圖示
description: GCP 架構圖示精選參考——216 個彩色服務圖示，適用於雲端基礎設施圖表與文件。
sidebar:
  label: GCP
  order: 16
i18n:
...
[GCP 架構圖示](https://cloud.google.com/icons) 提供 216 個彩色服務圖示，用於記錄 Google Cloud 基礎設施。使用來自 `@f5-sales-demo/icons-gcp` 的 `Icon` 元件，透過名稱渲染任意圖示。預設尺寸為 24x24，並內嵌填色。
...
這些圖示使用原廠商色彩（而非 `currentColor`）。色彩內嵌於 SVG 標記中，在淺色與深色模式下均完整保留。
...
如需完整的 216 個 GCP 架構圖示清單，請造訪 [Google Cloud 架構圖示](https://cloud.google.com/icons) 頁面。

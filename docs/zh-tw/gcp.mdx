---
title: GCP 架構圖示
description: GCP 架構圖示精選參考資料 — 216 個彩色服務圖示，適用於雲端基礎架構圖表與文件。
sidebar:
  label: GCP
  order: 16
i18n:
...
[GCP 架構圖示](https://cloud.google.com/icons) 提供 216 個彩色服務圖示，用於記錄 Google Cloud 基礎架構。使用 `@f5xc-salesdemos/icons-gcp` 中的 `Icon` 元件，依名稱渲染任何圖示。預設尺寸為 24x24，並採用內嵌填色。
...
這些圖示使用原廠原始色彩（非 `currentColor`）。顏色已嵌入 SVG 標記中，並在亮色與暗色模式下均保持不變。

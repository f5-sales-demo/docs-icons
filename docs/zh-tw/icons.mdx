...
所有圖示套件都使用各自套件的 `Icon.astro` 元件來渲染內嵌 SVG。每個套件都有自己的匯入方式：
...
F5 XC 服務圖示使用 CSS 自訂屬性，會自動適應淺色和深色模式 — 無需特別的類別處理。對於其他多色圖示（如 HashiCorp Flight 彩色變體），`.icon-card-image` 上的 `no-invert` 類別可在深色模式中保留原始顏色。
...
| F5 產品圖表 | F5 品牌 | 領域特定的網路/安全圖形 |
| 現代線條圖示 | Lucide | 簡潔一致的 1,600+ 圖示 |
| 大量填充/描邊圖示 | Material Design (mdi) | 最大的圖示集，含 7,638 個圖示 |
| IBM 設計語言 | Carbon | 2,582 個企業級圖示 |
| 靈活的粗細選項 | Phosphor | 9,161 個圖示，含六種粗細變體 |
| 清晰的線條圖示 | Tabler | 6,034 個圖示，一致的 2px 描邊 |
| F5 XC 服務圖表 | F5 XC 服務 | 30 個分散式雲端服務圖示 |
| 品牌/公司標誌 | Simple Icons | 3,200+ 個供應商和服務的品牌圖示 |
| 雲端/基礎架構供應商標誌 | HashiCorp Flight | AWS、GCP、Azure、K8s 供應商圖示 |
| AWS 架構圖表 | AWS 架構 | 885 個 AWS 基礎架構彩色服務圖示 |
| Azure 架構圖表 | Azure 架構 | 606 個 Azure 基礎架構彩色服務圖示 |
| GCP 架構圖表 | GCP 架構 | 216 個 Google Cloud 基礎架構彩色服務圖示 |

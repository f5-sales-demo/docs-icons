---
title: Phosphor 圖示
description: 'Phosphor 圖示精選參考資料 — 透過 Icon 元件提供 9,161 個具備彈性粗細變體的圖示。'
sidebar:
  label: Phosphor
  order: 8
i18n:
...
[Phosphor Icons](https://icon-sets.iconify.design/ph/) 提供 9,161 個圖示，支援彈性的粗細選項 — regular、bold、duotone、fill、light 及 thin。使用 `@f5xc-salesdemos/icons-phosphor` 中的 `Icon` 元件，透過名稱渲染任意圖示。
...
圖示以內嵌 SVG 的形式渲染，使用 `currentColor` — 可自動適應淺色與深色模式。

:::note[粗細變體]
Phosphor 圖示提供六種粗細。regular 粗細使用基本名稱（例如 `globe`）。其他粗細則附加後綴：`globe-bold`、`globe-duotone`、`globe-fill`、`globe-light`、`globe-thin`。以下範例顯示的是 regular 粗細。
:::
...
如需查看完整的 9,161 個 Phosphor 圖示（涵蓋所有粗細變體），請造訪 [Iconify Phosphor 探索器](https://icon-sets.iconify.design/ph/)。

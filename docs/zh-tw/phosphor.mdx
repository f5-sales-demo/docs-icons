---
title: Phosphor 圖示
description: 'Phosphor Icons 精選參考資料 — 透過 Icon 元件提供 9,161 個圖示及彈性粗細變體。'
sidebar:
  label: Phosphor
  order: 8
i18n:
...
[Phosphor Icons](https://icon-sets.iconify.design/ph/) 提供 9,161 個圖示，並支援彈性粗細選項 — regular、bold、duotone、fill、light 與 thin。使用來自 `@f5-sales-demo/icons-phosphor` 的 `Icon` 元件，透過名稱渲染任意圖示。
...
:::note[粗細變體]
Phosphor 圖示共有六種粗細。regular 粗細使用基本名稱（例如 `globe`）。其他粗細則在名稱後附加後綴：`globe-bold`、`globe-duotone`、`globe-fill`、`globe-light`、`globe-thin`。以下範例展示的是 regular 粗細。
:::
...
如需完整的 9,161 個 Phosphor Icons 清單（涵蓋所有粗細變體），請造訪 [Iconify Phosphor 瀏覽器](https://icon-sets.iconify.design/ph/)。

---
title: Phosphor 圖示
description: 'Phosphor Icons 精選參考 — 9,161 個圖示，提供靈活的粗細變體，可透過 Icon 元件使用。'
sidebar:
  label: Phosphor
  order: 8
i18n:
...
[Phosphor Icons](https://icon-sets.iconify.design/ph/) 提供 9,161 個圖示，具有靈活的粗細選項 — regular、bold、duotone、fill、light 和 thin。使用 `@f5xc-salesdemos/icons-phosphor` 中的 `Icon` 元件，即可透過名稱渲染任何圖示。
...
:::note[粗細變體]
Phosphor 圖示有六種粗細。regular 粗細使用基本名稱（例如 `globe`）。其他粗細會附加後綴：`globe-bold`、`globe-duotone`、`globe-fill`、`globe-light`、`globe-thin`。以下範例展示的是 regular 粗細。
:::
...
如需查看完整的 9,161 個 Phosphor Icons 清單（涵蓋所有粗細變體），請造訪 [Iconify Phosphor 瀏覽器](https://icon-sets.iconify.design/ph/)。

---
title: Tabler 圖示
description: 'Tabler Icons 精選參考 — 6,034 個清晰的線條圖示，具有一致的筆畫寬度，可透過 Icon 元件使用。'
sidebar:
  label: Tabler
  order: 9
i18n:
...
[Tabler Icons](https://icon-sets.iconify.design/tabler/) 提供 6,034 個清晰的線條圖示，具有一致的 2px 筆畫寬度。使用 `@f5xc-salesdemos/icons-tabler` 中的 `Icon` 元件，透過名稱來渲染任何圖示。
...
圖示會以內嵌 SVG 的形式渲染，使用 `currentColor` — 它們會自動適應亮色和暗色模式。
...
如需查看完整的 6,034 個 Tabler Icons 清單，請造訪 [Iconify Tabler 瀏覽器](https://icon-sets.iconify.design/tabler/)。

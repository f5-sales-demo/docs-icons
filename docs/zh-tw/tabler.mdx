---
title: Tabler Icons
description: 'Tabler Icons 精選參考 — 6,034 個清晰的線條圖示，具備一致的筆劃寬度，可透過 Icon 元件使用。'
sidebar:
  label: Tabler
  order: 9
i18n:
...
[Tabler Icons](https://icon-sets.iconify.design/tabler/) 提供 6,034 個清晰的線條圖示，具備一致的 2px 筆劃寬度。使用 `@f5xc-salesdemos/icons-tabler` 中的 `Icon` 元件，即可透過名稱渲染任何圖示。
...
圖示以內嵌 SVG 的方式渲染，使用 `currentColor` — 它們會自動適應淺色和深色模式。

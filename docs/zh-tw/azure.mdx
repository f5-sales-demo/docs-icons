---
title: Azure 架構圖示
description: Azure 架構圖示精選參考手冊 — 606 個彩色服務圖示，適用於雲端基礎架構圖表與文件。
sidebar:
  label: Azure
  order: 15
i18n:
...
[Azure 架構圖示](https://learn.microsoft.com/en-us/azure/architecture/icons/) 提供 606 個彩色服務圖示，用於記錄 Azure 基礎架構。使用 `@f5xc-salesdemos/icons-azure` 中的 `Icon` 元件，可依名稱渲染任意圖示。預設尺寸為 18x18，並採用漸層填色。
...
這些圖示使用原廠商顏色（非 `currentColor`）。顏色已嵌入 SVG 標記中，並在淺色與深色模式下均予以保留。

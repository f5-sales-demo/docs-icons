...
description: Azure Architecture Icons 的精選參考資料 — 606 個彩色服務圖示，用於雲端基礎架構圖表與文件製作。
sidebar:
  label: Azure
  order: 15
i18n:
...
[Azure Architecture Icons](https://learn.microsoft.com/en-us/azure/architecture/icons/) 提供 606 個彩色服務圖示，用於記錄 Azure 基礎架構。使用來自 `@f5-sales-demo/icons-azure` 的 `Icon` 元件，可依名稱渲染任何圖示。預設尺寸為 18x18，並帶有漸層填色。
...
這些圖示使用原廠商顏色（而非 `currentColor`）。顏色嵌入於 SVG 標記中，並在淺色與深色模式下均可保留。
...
如需完整的 606 個 Azure Architecture Icons 清單，請造訪 [Microsoft Azure Architecture Icons](https://learn.microsoft.com/en-us/azure/architecture/icons/) 頁面。

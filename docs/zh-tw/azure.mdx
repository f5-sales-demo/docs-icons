...
description: Azure Architecture Icons 精選參考 — 606 個彩色服務圖示，適用於雲端基礎架構圖表與文件。
sidebar:
  label: Azure
  order: 15
i18n:
...
[Azure Architecture Icons](https://learn.microsoft.com/en-us/azure/architecture/icons/) 提供 606 個彩色服務圖示，用於記錄 Azure 基礎架構。使用 `@f5xc-salesdemos/icons-azure` 中的 `Icon` 元件，可按名稱呈現任何圖示。預設大小為 18x18，使用漸層填充。
...
這些圖示使用原始供應商色彩（非 `currentColor`）。色彩嵌入於 SVG 標記中，在淺色和深色模式下均會保留。

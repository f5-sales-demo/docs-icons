...
[Azure 架構圖示](https://learn.microsoft.com/en-us/azure/architecture/icons/)提供 606 個彩色服務圖示，用於記錄 Azure 基礎架構。使用 `@f5xc-salesdemos/icons-azure` 中的 `Icon` 元件，可透過名稱呈現任何圖示。預設大小為 18x18，並帶有漸層填色。
...
這些圖示使用原始供應商色彩（而非 `currentColor`）。色彩嵌入於 SVG 標記中，並在淺色與深色模式下皆會保留。

...
description: 'Material Design Icons 精選參考資料 — 7,638 個填充與外框圖示，可透過 Icon 元件使用。'
sidebar:
...
[Material Design Icons](https://icon-sets.iconify.design/mdi/) 提供 7,638 個圖示，涵蓋填充與外框兩種變體，幾乎適用於所有使用情境。使用 `@f5-sales-demo/icons-mdi` 中的 `Icon` 元件，透過名稱渲染任意圖示。
...
圖示以內嵌 SVG 方式渲染，使用 `currentColor` — 可自動適應亮色與暗色模式。
...
如需完整的 7,638 個 Material Design Icons 清單，請造訪 [Iconify MDI 瀏覽器](https://icon-sets.iconify.design/mdi/)。

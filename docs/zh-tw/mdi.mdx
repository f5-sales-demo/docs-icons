...
description: 'Material Design Icons 精選參考 — 7,638 個填滿與輪廓圖示，可透過 Icon 元件使用。'
sidebar:
...
[Material Design Icons](https://icon-sets.iconify.design/mdi/) 提供 7,638 個圖示，包含填滿與輪廓變體，幾乎涵蓋所有使用情境。使用 `@f5xc-salesdemos/icons-mdi` 中的 `Icon` 元件，即可透過名稱渲染任何圖示。
...
圖示以內嵌 SVG 方式渲染，使用 `currentColor` — 會自動適應淺色與深色模式。

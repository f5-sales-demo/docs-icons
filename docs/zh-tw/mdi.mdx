...
[Material Design Icons](https://icon-sets.iconify.design/mdi/) 提供 7,638 個圖示，包含填充與輪廓變體，幾乎涵蓋所有使用情境。使用 `@f5xc-salesdemos/icons-mdi` 中的 `Icon` 元件，即可透過名稱呈現任何圖示。
...
圖示以內嵌 SVG 方式呈現，使用 `currentColor` — 會自動適應淺色與深色模式。
...
如需查看完整的 7,638 個 Material Design Icons 列表，請造訪 [Iconify MDI 瀏覽器](https://icon-sets.iconify.design/mdi/)。

...
[Carbon Icons](https://icon-sets.iconify.design/carbon/) 提供 2,582 個來自 IBM 企業設計語言的簡潔線框圖示。使用 `@f5xc-salesdemos/icons-carbon` 中的 `Icon` 元件，即可透過名稱渲染任何圖示。
...
圖示會以內嵌 SVG 的形式渲染，並使用 `currentColor` — 它們會自動適應淺色和深色模式。
...
如需查看完整的 2,582 個 Carbon Icons 清單，請造訪 [Iconify Carbon 瀏覽器](https://icon-sets.iconify.design/carbon/)。

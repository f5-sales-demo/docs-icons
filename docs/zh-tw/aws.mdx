---
title: AWS 架構圖示
description: AWS 架構圖示精選參考資料 — 885 個彩色服務圖示，適用於雲端基礎設施圖表與文件。
sidebar:
  label: AWS
  order: 14
i18n:
...
[AWS 架構圖示](https://aws.amazon.com/architecture/icons/) 提供 885 個彩色服務圖示，用於記錄 AWS 基礎設施。使用 `@f5xc-salesdemos/icons-aws` 中的 `Icon` 元件，透過名稱渲染任意圖示。預設尺寸為 48x48，可針對個別圖示覆寫大小。
...
這些圖示使用原廠商色彩（而非 `currentColor`）。色彩嵌入於 SVG 標記中，並在明亮模式與深色模式下均保持不變。
...
如需完整的 885 個 AWS 架構圖示清單，請造訪 [AWS 架構圖表圖示](https://aws.amazon.com/architecture/icons/)。

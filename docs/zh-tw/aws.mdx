---
title: AWS 架構圖示
description: AWS 架構圖示精選參考 — 885 個彩色服務圖示，適用於雲端基礎設施圖表與文件。
sidebar:
  label: AWS
  order: 14
i18n:
...
[AWS Architecture Icons](https://aws.amazon.com/architecture/icons/) 提供 885 個彩色服務圖示，用於記錄 AWS 基礎設施。使用來自 `@f5xc-salesdemos/icons-aws` 的 `Icon` 元件，即可透過名稱呈現任何圖示。預設大小為 48x48，並支援個別圖示的尺寸覆寫。
...
這些圖示使用原廠供應商色彩（非 `currentColor`）。色彩嵌入於 SVG 標記中，在淺色與深色模式下均會保留。

---
title: AWS 架構圖示
description: AWS 架構圖示的精選參考 — 885 個彩色服務圖示，適用於雲端基礎架構圖表與文件。
sidebar:
  label: AWS
  order: 14
i18n:
...
[AWS Architecture Icons](https://aws.amazon.com/architecture/icons/) 提供 885 個彩色服務圖示，用於記錄 AWS 基礎架構。使用 `@f5xc-salesdemos/icons-aws` 中的 `Icon` 元件，透過名稱來渲染任何圖示。預設大小為 48x48，並支援針對個別圖示的大小覆寫。
...
這些圖示使用原始供應商顏色（非 `currentColor`）。顏色嵌入在 SVG 標記中，在淺色和深色模式下均會保留。
...
如需完整的 885 個 AWS 架構圖示清單，請造訪 [AWS Icons for Architecture Diagrams](https://aws.amazon.com/architecture/icons/)。

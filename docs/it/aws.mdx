...
description: >-
  Riferimento curato per le Icone di Architettura AWS — 885 icone di servizio a
  colori per diagrammi e documentazione dell'infrastruttura cloud.
sidebar:
  label: AWS
  order: 14
i18n:
...
[AWS Architecture Icons](https://aws.amazon.com/architecture/icons/) fornisce 885 icone di servizio a colori per documentare l'infrastruttura AWS. Utilizzare il componente `Icon` da `@f5xc-salesdemos/icons-aws` per renderizzare qualsiasi icona per nome. La dimensione predefinita è 48x48 con possibilità di sovrascrittura della dimensione per singola icona.

---
title: Icone dell'architettura AWS
description: >-
  Riferimento curato per le icone dell'architettura AWS — 885 icone a colori dei
...
[AWS Architecture Icons](https://aws.amazon.com/architecture/icons/) fornisce 885 icone a colori dei servizi per documentare l'infrastruttura AWS. Utilizzate il componente `Icon` da `@f5xc-salesdemos/icons-aws` per renderizzare qualsiasi icona per nome. La dimensione predefinita è 48x48 con possibilità di sovrascrittura per singola icona.
...
Per l'elenco completo delle 885 icone dell'architettura AWS, visitate [AWS Icons for Architecture Diagrams](https://aws.amazon.com/architecture/icons/).

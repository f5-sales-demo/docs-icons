...
description: >-
  Riferimento curato per le icone di architettura AWS — 885 icone colorate dei
  servizi per diagrammi e documentazione dell'infrastruttura cloud.
sidebar:
  label: AWS
  order: 14
i18n:
...
[AWS Architecture Icons](https://aws.amazon.com/architecture/icons/) fornisce 885 icone colorate dei servizi per documentare l'infrastruttura AWS. Utilizzate il componente `Icon` da `@f5xc-salesdemos/icons-aws` per renderizzare qualsiasi icona per nome. La dimensione predefinita è 48x48 con possibilità di sovrascrivere la dimensione per singola icona.
...
Per l'elenco completo delle 885 icone di architettura AWS, visitate [AWS Icons for Architecture Diagrams](https://aws.amazon.com/architecture/icons/).

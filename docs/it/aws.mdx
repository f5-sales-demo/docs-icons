---
title: Icone di architettura AWS
description: >-
  Riferimento curato per le icone di architettura AWS — 885 icone a colori dei
  servizi per diagrammi di infrastruttura cloud e documentazione.
sidebar:
  label: AWS
  order: 14
i18n:
...
[AWS Architecture Icons](https://aws.amazon.com/architecture/icons/) fornisce 885 icone a colori dei servizi per documentare l'infrastruttura AWS. Utilizzare il componente `Icon` da `@f5-sales-demo/icons-aws` per renderizzare qualsiasi icona tramite nome. La dimensione predefinita è 48x48 con possibilità di sovrascrittura per singola icona.
...
Per l'elenco completo delle 885 icone di architettura AWS, visitare [AWS Icons for Architecture Diagrams](https://aws.amazon.com/architecture/icons/).

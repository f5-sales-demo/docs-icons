---
title: Icone per l'architettura Azure
description: >-
  Riferimento curato per le icone dell'architettura Azure — 606 icone a colori
  per i servizi, destinate a diagrammi e documentazione dell'infrastruttura
  cloud.
sidebar:
  label: Azure
  order: 15
...
[Azure Architecture Icons](https://learn.microsoft.com/en-us/azure/architecture/icons/) fornisce 606 icone a colori per i servizi, utili per documentare l'infrastruttura Azure. Utilizzate il componente `Icon` da `@f5xc-salesdemos/icons-azure` per renderizzare qualsiasi icona tramite il suo nome. La dimensione predefinita è 18x18 con riempimenti a gradiente.
...
Queste icone utilizzano i colori originali del fornitore (non `currentColor`). I colori sono incorporati nel markup SVG e vengono preservati sia in modalità chiara che scura.
...
## Esplora tutte le icone

Per l'elenco completo delle 606 icone dell'architettura Azure, visitate la pagina [Microsoft Azure Architecture Icons](https://learn.microsoft.com/en-us/azure/architecture/icons/).

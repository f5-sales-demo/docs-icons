---
title: Icone integrate di Starlight
description: >-
  Riferimento completo per tutte le oltre 200 icone integrate di Starlight
  disponibili tramite il componente Icon.
sidebar:
  label: Integrate di Starlight
  order: 3
i18n:
...
Starlight include oltre 200 icone integrate disponibili tramite il componente `<Icon>` — senza necessità di installazione. Il plugin `starlight-plugin-icons` è già configurato nel tema, quindi queste icone funzionano in qualsiasi repository di contenuti.

## Utilizzo del componente Icon

Importate il componente e passate il nome di un'icona:
...
|------|------|---------|-------------|
| `name` | `string` | — | **Obbligatoria.** Il nome dell'icona dalle tabelle sottostanti. |
| `label` | `string` | — | Etichetta accessibile per gli screen reader. Usatela per icone significative; omettetela per quelle decorative. |
| `size` | `string` | `"1em"` | Valore CSS di dimensione applicato a larghezza e altezza. |
| `color` | `string` | `"currentColor"` | Valore CSS di colore. |
| `class` | `string` | — | Classi CSS aggiuntive. |

:::tip[Accessibilità]
Aggiungete una proprietà `label` quando un'icona trasmette un significato (ad esempio un'icona di collegamento). Omettete `label` per le icone puramente decorative — Starlight le nasconderà automaticamente alle tecnologie assistive.
:::
...
Le icone Seti utilizzano il prefisso `seti:` e sono progettate per la rappresentazione dei tipi di file. Funzionano nel componente `<Icon>` e nel componente `<FileTree>`.

...
Importa il componente e passa il nome di un'icona:
...
|------|------|---------|-------------|
| `name` | `string` | — | **Obbligatorio.** Il nome dell'icona dalle tabelle sottostanti. |
| `label` | `string` | — | Etichetta accessibile per gli screen reader. Usare per le icone significative; omettere per quelle decorative. |
| `size` | `string` | `"1em"` | Valore di dimensione CSS applicato a larghezza e altezza. |
| `color` | `string` | `"currentColor"` | Valore di colore CSS. |
| `class` | `string` | — | Classi CSS aggiuntive. |

:::tip[Accessibilità]
Aggiungi la proprietà `label` quando un'icona trasmette un significato (es. un'icona di collegamento). Ometti `label` per le icone puramente decorative — Starlight le nasconderà automaticamente dalle tecnologie assistive.
:::
...
Le icone Seti utilizzano il prefisso `seti:` e sono progettate per la rappresentazione dei tipi di file. Funzionano sia nel componente `<Icon>` che nel componente `<FileTree>`.

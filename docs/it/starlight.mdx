...
sidebar:
  label: Icone integrate di Starlight
  order: 3
i18n:
...
Starlight include oltre 200 icone integrate disponibili tramite il componente `<Icon>` — nessuna installazione necessaria. Il plugin `starlight-plugin-icons` è già configurato nel tema, quindi queste icone funzionano in qualsiasi repository di contenuti.
...
| `label` | `string` | — | Etichetta accessibile per gli screen reader. Da usare per icone significative; da omettere per quelle decorative. |
...
:::tip[Accessibilità]
Aggiungi la prop `label` quando un'icona trasmette un significato (ad es. un'icona di link). Ometti `label` per le icone puramente decorative — Starlight le nasconderà automaticamente alle tecnologie assistive.
:::

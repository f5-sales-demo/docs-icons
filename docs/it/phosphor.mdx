...
[Phosphor Icons](https://icon-sets.iconify.design/ph/) fornisce 9.161 icone con opzioni di peso flessibili — regular, bold, duotone, fill, light e thin. Utilizza il componente `Icon` da `@f5xc-salesdemos/icons-phosphor` per renderizzare qualsiasi icona per nome.
...
Le icone Phosphor sono disponibili in sei pesi. Il peso regular utilizza il nome base (ad es. `globe`). Gli altri pesi aggiungono un suffisso: `globe-bold`, `globe-duotone`, `globe-fill`, `globe-light`, `globe-thin`. Gli esempi seguenti mostrano il peso regular.
:::
...
Per l'elenco completo delle 9.161 icone Phosphor (in tutte le varianti di peso), visita l'[esploratore Phosphor di Iconify](https://icon-sets.iconify.design/ph/).

---
title: Icone Phosphor
description: >-
  Riferimento curato per le Icone Phosphor — 9.161 icone con varianti di
  spessore flessibili disponibili tramite il componente Icon.
sidebar:
  label: Phosphor
  order: 8
i18n:
...
[Icone Phosphor](https://icon-sets.iconify.design/ph/) fornisce 9.161 icone con opzioni di spessore flessibili — regular, bold, duotone, fill, light e thin. Utilizzare il componente `Icon` da `@f5xc-salesdemos/icons-phosphor` per visualizzare qualsiasi icona per nome.
...
Le icone vengono visualizzate come SVG inline usando `currentColor` — si adattano automaticamente alla modalità chiara e scura.

:::note[Varianti di spessore]
Le icone Phosphor sono disponibili in sei spessori. Lo spessore regular utilizza il nome base (ad es. `globe`). Gli altri spessori aggiungono un suffisso: `globe-bold`, `globe-duotone`, `globe-fill`, `globe-light`, `globe-thin`. Gli esempi seguenti mostrano lo spessore regular.
:::
...
Per l'elenco completo delle 9.161 Icone Phosphor (in tutte le varianti di spessore), visitare l'[explorer Iconify Phosphor](https://icon-sets.iconify.design/ph/).

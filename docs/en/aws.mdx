...
[AWS Architecture Icons](https://aws.amazon.com/architecture/icons/) provides 885 color service icons for documenting AWS infrastructure. Use the `Icon` component from `@f5-sales-demo/icons-aws` to render any icon by name. Default size is 48x48 with per-icon size overrides.

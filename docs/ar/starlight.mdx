...
يتضمن Starlight أكثر من 200 أيقونة مدمجة متاحة عبر مكون `<Icon>` — دون الحاجة إلى أي تثبيت. إضافة `starlight-plugin-icons` مُعدّة مسبقاً في القالب، لذا تعمل هذه الأيقونات في أي مستودع محتوى.

## استخدام مكون الأيقونة

قم باستيراد المكون وتمرير اسم الأيقونة:
...
| `size` | `string` | `"1em"` | قيمة حجم CSS تُطبق على العرض والارتفاع. |
| `color` | `string` | `"currentColor"` | قيمة لون CSS. |
| `class` | `string` | — | فئات CSS إضافية. |

:::tip[إمكانية الوصول]
أضف خاصية `label` عندما تنقل الأيقونة معنى معيناً (مثل أيقونة رابط). احذف `label` للأيقونات الزخرفية البحتة — سيقوم Starlight بإخفائها عن تقنيات المساعدة تلقائياً.
:::
...
تستخدم أيقونات Seti البادئة `seti:` وهي مصممة لتمثيل أنواع الملفات. تعمل في مكون `<Icon>` ومكون `<FileTree>`.

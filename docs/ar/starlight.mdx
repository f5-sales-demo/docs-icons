---
title: أيقونات Starlight المدمجة
description: >-
  مرجع شامل لجميع الأيقونات المدمجة في Starlight والتي يزيد عددها عن 200 أيقونة
  متاحة عبر مكوّن Icon.
sidebar:
  label: أيقونات Starlight المدمجة
  order: 3
i18n:
...
يتضمن Starlight أكثر من 200 أيقونة مدمجة متاحة عبر مكوّن `<Icon>` — دون الحاجة إلى أي تثبيت. إضافة `starlight-plugin-icons` مُهيّأة مسبقاً في القالب، لذا تعمل هذه الأيقونات في أي مستودع محتوى.

## استخدام مكوّن الأيقونة

قم باستيراد المكوّن ومرّر اسم الأيقونة:
...
|------|------|---------|-------------|
| `name` | `string` | — | **مطلوبة.** اسم الأيقونة من الجداول أدناه. |
| `label` | `string` | — | تسمية إمكانية الوصول لقارئات الشاشة. استخدمها للأيقونات ذات المعنى؛ واحذفها للأيقونات الزخرفية. |
| `size` | `string` | `"1em"` | قيمة حجم CSS تُطبّق على العرض والارتفاع. |
| `color` | `string` | `"currentColor"` | قيمة لون CSS. |
| `class` | `string` | — | فئات CSS إضافية. |

:::tip[إمكانية الوصول]
أضف خاصية `label` عندما تنقل الأيقونة معنى (مثل أيقونة رابط). احذف `label` للأيقونات الزخرفية البحتة — سيخفيها Starlight تلقائياً عن تقنيات المساعدة.
:::
...
## أيقونات التواصل الاجتماعي والمجتمع
...
تستخدم أيقونات Seti البادئة `seti:` وهي مصممة لتمثيل أنواع الملفات. تعمل في مكوّن `<Icon>` ومكوّن `<FileTree>`.

...
:::note[Dunkelmodus]
Diese mehrfarbigen Icons verwenden CSS Custom Properties (`--color-N600`, `--color-brand`, `--color-blue-light`, `--color-N200`), die sich automatisch an den hellen und dunklen Modus anpassen. Navy und Hellblau werden im Dunkelmodus getauscht, während der F5-Rot-Akzent konstant bleibt.
:::

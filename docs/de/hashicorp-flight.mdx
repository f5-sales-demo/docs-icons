...
Icons werden als Inline-SVGs mit `currentColor` gerendert — monochrome Icons passen sich automatisch an den Hell- und Dunkelmodus an. Farbvarianten behalten ihre Originalfarben bei.

:::note[Dunkelmodus]
Monochrome Icons verwenden `currentColor` und passen sich automatisch an. Farbvarianten (Icons mit der Endung `-color`) behalten ihre Originalfarben über die `no-invert`-Klasse bei.
:::

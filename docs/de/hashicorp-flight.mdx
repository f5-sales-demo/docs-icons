...
description: >-
  Visuelle Referenz für alle 672 HashiCorp Flight Icons aus
...
Vollständige visuelle Referenz für jedes Symbol im [`@f5-sales-demo/icons-hashicorp-flight`](https://www.npmjs.com/package/@f5-sales-demo/icons-hashicorp-flight)-Paket. Symbole werden über die `Icon`-Komponente gerendert und nach Kategorie geordnet.
...
Symbole werden als Inline-SVGs mit `currentColor` gerendert — monochrome Symbole passen sich automatisch an den Hell- und Dunkelmodus an. Farbvarianten behalten ihre ursprünglichen Farben bei.

:::note[Dunkelmodus]
Monochrome Symbole verwenden `currentColor` und passen sich automatisch an. Farbvarianten (Symbole, die auf `-color` enden) behalten ihre ursprünglichen Farben über die Klasse `no-invert` bei.
:::

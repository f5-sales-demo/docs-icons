...
Vollständige visuelle Referenz für jedes Icon im Paket [`@f5xc-salesdemos/icons-hashicorp-flight`](https://www.npmjs.com/package/@f5xc-salesdemos/icons-hashicorp-flight). Icons werden über die `Icon`-Komponente gerendert und nach Kategorie organisiert.
...
Icons werden als Inline-SVGs mit `currentColor` gerendert — einfarbige Icons passen sich automatisch an den Hell- und Dunkelmodus an. Farbvarianten behalten ihre Originalfarben bei.

:::note[Dunkelmodus]
Einfarbige Icons verwenden `currentColor` und passen sich automatisch an. Farbvarianten (Icons mit der Endung `-color`) behalten ihre Originalfarben über die Klasse `no-invert` bei.
:::

...
Vollständige visuelle Referenz für jedes Symbol im Paket [`@f5xc-salesdemos/icons-hashicorp-flight`](https://www.npmjs.com/package/@f5xc-salesdemos/icons-hashicorp-flight). Symbole werden über die `Icon`-Komponente gerendert und nach Kategorie geordnet.
...
:::note[Dunkelmodus]
Monochrome Symbole verwenden `currentColor` und passen sich automatisch an. Farbvarianten (Symbole mit der Endung `-color`) behalten ihre ursprünglichen Farben über die Klasse `no-invert` bei.
:::

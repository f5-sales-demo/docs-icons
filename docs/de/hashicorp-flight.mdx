...
Vollständige visuelle Referenz für jedes Icon im [`@f5xc-salesdemos/icons-hashicorp-flight`](https://www.npmjs.com/package/@f5xc-salesdemos/icons-hashicorp-flight)-Paket. Icons werden über die `Icon`-Komponente gerendert und nach Kategorie organisiert.
...
Icons werden als Inline-SVGs mit `currentColor` gerendert — monochrome Icons passen sich automatisch an den Hell- und Dunkelmodus an. Farbvarianten behalten ihre ursprünglichen Farben bei.

:::note[Dunkelmodus]
Monochrome Icons verwenden `currentColor` und passen sich automatisch an. Farbvarianten (Icons mit der Endung `-color`) behalten ihre ursprünglichen Farben über die `no-invert`-Klasse bei.
:::

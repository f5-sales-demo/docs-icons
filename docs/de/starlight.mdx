...
description: >-
  Vollständige Referenz für alle über 200 integrierten Starlight-Icons, die über
  die Icon-Komponente verfügbar sind.
sidebar:
...
| `label` | `string` | — | Barrierefreie Bezeichnung für Screenreader. Verwenden Sie diese für bedeutungstragende Icons; lassen Sie sie bei dekorativen Icons weg. |
...
Fügen Sie eine `label`-Prop hinzu, wenn ein Icon eine Bedeutung vermittelt (z. B. ein Link-Icon). Lassen Sie `label` bei rein dekorativen Icons weg — Starlight verbirgt sie automatisch vor assistiven Technologien.
:::

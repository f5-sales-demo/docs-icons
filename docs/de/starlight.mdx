---
title: Integrierte Starlight-Icons
description: >-
  Vollständige Referenz für alle 200+ integrierten Starlight-Icons, die über die
  Icon-Komponente verfügbar sind.
sidebar:
  label: Integrierte Starlight-Icons
  order: 3
i18n:
...
Starlight enthält über 200 integrierte Icons, die über die `<Icon>`-Komponente verfügbar sind — keine Installation erforderlich. Das `starlight-plugin-icons`-Plugin ist bereits im Theme konfiguriert, sodass diese Icons in jedem Content-Repository funktionieren.

## Verwendung der Icon-Komponente

Importieren Sie die Komponente und übergeben Sie einen Icon-Namen:
...
|------|------|---------|-------------|
| `name` | `string` | — | **Erforderlich.** Der Icon-Name aus den untenstehenden Tabellen. |
| `label` | `string` | — | Barrierefreie Beschriftung für Screenreader. Verwenden Sie diese für bedeutungstragende Icons; lassen Sie sie bei dekorativen Icons weg. |
| `size` | `string` | `"1em"` | CSS-Größenwert, der auf Breite und Höhe angewendet wird. |
| `color` | `string` | `"currentColor"` | CSS-Farbwert. |
| `class` | `string` | — | Zusätzliche CSS-Klassen. |

:::tip[Barrierefreiheit]
Fügen Sie eine `label`-Prop hinzu, wenn ein Icon eine Bedeutung vermittelt (z. B. ein Link-Icon). Lassen Sie `label` bei rein dekorativen Icons weg — Starlight verbirgt sie automatisch vor unterstützenden Technologien.
:::
...
Seti-Icons verwenden das Präfix `seti:` und sind für die Darstellung von Dateitypen konzipiert. Sie funktionieren sowohl in der `<Icon>`-Komponente als auch in der `<FileTree>`-Komponente.

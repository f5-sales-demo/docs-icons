...
description: 通过 Icon 组件可用的全部 200+ 个 Starlight 内置图标完整参考。
sidebar:
...
Starlight 包含超过 200 个内置图标，可通过 `<Icon>` 组件使用——无需额外安装。主题中已配置了 `starlight-plugin-icons` 插件，因此这些图标可在任何内容仓库中使用。
...
| `label` | `string` | — | 供屏幕阅读器使用的无障碍标签。用于有实际含义的图标；装饰性图标可省略。 |
| `size` | `string` | `"1em"` | 应用于宽度和高度的 CSS 尺寸值。 |
| `color` | `string` | `"currentColor"` | CSS 颜色值。 |
| `class` | `string` | — | 额外的 CSS 类名。 |

:::tip[无障碍]
当图标传达含义时（例如链接图标），请添加 `label` 属性。对于纯装饰性图标，请省略 `label`——Starlight 会自动将其对辅助技术隐藏。
:::

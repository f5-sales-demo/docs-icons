---
title: Tabler 图标
description: 'Tabler 图标精选参考——6,034 个线条清晰、描边宽度一致的图标，可通过 Icon 组件使用。'
sidebar:
  label: Tabler
  order: 9
i18n:
...
[Tabler Icons](https://icon-sets.iconify.design/tabler/) 提供 6,034 个线条清晰、描边宽度统一为 2px 的图标。使用 `@f5xc-salesdemos/icons-tabler` 中的 `Icon` 组件，通过名称渲染任意图标。
...
图标以内联 SVG 形式渲染，使用 `currentColor`——可自动适配亮色与暗色模式。

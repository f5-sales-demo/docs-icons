---
title: Tabler 图标
description: 'Tabler Icons 精选参考 — 6,034 个清晰的线条图标，笔画宽度一致，可通过 Icon 组件使用。'
sidebar:
  label: Tabler
  order: 9
i18n:
...
[Tabler Icons](https://icon-sets.iconify.design/tabler/) 提供 6,034 个清晰的线条图标，具有一致的 2px 笔画宽度。使用 `@f5xc-salesdemos/icons-tabler` 中的 `Icon` 组件，通过名称渲染任意图标。
...
图标以内联 SVG 的形式渲染，使用 `currentColor` — 它们会自动适配浅色和深色模式。
...
如需查看全部 6,034 个 Tabler 图标的完整列表，请访问 [Iconify Tabler 浏览器](https://icon-sets.iconify.design/tabler/)。

---
title: 图标参考
description: 文档主题中所有可用图标包的概览——用法、深色模式行为和选择指南。
sidebar:
  label: 概览
  order: 2
i18n:
...
文档主题提供了来自多个图标包的数千个图标。每个图标包使用相同的模式：导入该包的 `Icon.astro` 组件，然后通过名称引用图标。
...
所有图标包使用各自包内的 `Icon.astro` 组件来渲染内联 SVG。每个图标包有自己的导入方式：
...
图标使用 `currentColor`，会自动适应浅色和深色模式，无需特殊处理。
...
所有 `Icon` 组件使用 `currentColor` 渲染内联 SVG。它们会自动适应当前主题——无需特殊的 CSS 或类处理。

F5 XC 服务图标使用 CSS 自定义属性，会自动适应浅色和深色模式——无需特殊的类处理。对于其他多色图标（如 HashiCorp Flight 的彩色变体），`.icon-card-image` 上的 `no-invert` 类会在深色模式下保留原始颜色。
...
| F5 产品图表 | F5 品牌 | 针对网络/安全领域的专用图形 |
| 现代描边图标 | Lucide | 简洁一致，1,600+ 图标 |
| 大规模填充/描边图标 | Material Design (mdi) | 最大的图标集，含 7,638 个图标 |
...
| 品牌/公司徽标 | Simple Icons | 3,200+ 供应商和服务的品牌图标 |
| 云/基础设施供应商徽标 | HashiCorp Flight | AWS、GCP、Azure、K8s 供应商图标 |

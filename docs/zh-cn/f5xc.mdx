---
title: F5 XC
description: 来自 @f5xc-salesdemos/icons-f5xc 的全部 30 个 F5 Distributed Cloud (XC) 服务图标的可视化参考。
sidebar:
  label: F5 XC
  order: 13
i18n:
...
[`@f5xc-salesdemos/icons-f5xc`](https://www.npmjs.com/package/@f5xc-salesdemos/icons-f5xc) 包中每个图标的完整可视化参考。这 30 个图标代表 F5 Distributed Cloud 服务，以 Iconify JSON 格式提供，并附带 Astro 组件。
...
:::note[深色模式]
这些多色图标使用 CSS 自定义属性（`--color-N600`、`--color-brand`、`--color-blue-light`、`--color-N200`），可自动适配浅色和深色模式。深色模式下，深蓝色和浅蓝色会互换，而 F5 红色强调色保持不变。
:::

---
title: GCP 架构图标
description: GCP 架构图标精选参考 — 216 个彩色服务图标，适用于云基础设施架构图和文档。
sidebar:
  label: GCP
  order: 16
i18n:
...
[GCP 架构图标](https://cloud.google.com/icons)提供了 216 个彩色服务图标，用于记录 Google Cloud 基础设施。使用 `@f5xc-salesdemos/icons-gcp` 中的 `Icon` 组件，可以按名称渲染任意图标。默认大小为 24x24，使用内联填充。
...
这些图标使用供应商原始颜色（非 `currentColor`）。颜色嵌入在 SVG 标记中，在浅色和深色模式下均保持不变。
...
如需查看全部 216 个 GCP 架构图标的完整列表，请访问 [Google Cloud 架构图标](https://cloud.google.com/icons)页面。

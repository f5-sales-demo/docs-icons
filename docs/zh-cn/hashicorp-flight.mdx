---
title: HashiCorp Flight 图标
description: >-
  来自 @f5-sales-demo/icons-hashicorp-flight 的全部 672 个 HashiCorp Flight
  图标的可视化参考。
sidebar:
...
[`@f5-sales-demo/icons-hashicorp-flight`](https://www.npmjs.com/package/@f5-sales-demo/icons-hashicorp-flight) 包中每个图标的完整可视化参考。图标通过 `Icon` 组件渲染，并按类别组织。
...
图标以内联 SVG 的形式使用 `currentColor` 渲染——单色图标可自动适应浅色和深色模式。彩色变体保留其原始颜色。

:::note[深色模式]
单色图标使用 `currentColor`，可自动适应。彩色变体（以 `-color` 结尾的图标）通过 `no-invert` 类保留其原始颜色。
:::

...
[`@f5xc-salesdemos/icons-hashicorp-flight`](https://www.npmjs.com/package/@f5xc-salesdemos/icons-hashicorp-flight) 包中所有图标的完整可视化参考。图标通过 `Icon` 组件渲染，并按类别组织。
...
图标以内联 SVG 形式渲染，使用 `currentColor` — 单色图标会自动适应浅色和深色模式。彩色变体保留其原始颜色。

:::note[深色模式]
单色图标使用 `currentColor` 并自动适应。彩色变体（以 `-color` 结尾的图标）通过 `no-invert` 类保留其原始颜色。
:::

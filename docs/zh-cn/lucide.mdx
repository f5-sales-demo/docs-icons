---
title: Lucide 图标
description: 'Lucide 图标精选参考 — 通过 Icon 组件提供 1,600 多个现代描边图标。'
sidebar:
  label: Lucide
  order: 5
i18n:
...
[Lucide](https://lucide.dev/) 提供约 1,600 个现代、风格一致的描边图标。使用 `@f5xc-salesdemos/icons-lucide` 中的 `Icon` 组件，通过名称渲染任意图标。
...
图标以内联 SVG 方式渲染，使用 `currentColor` — 可自动适配浅色和深色模式。

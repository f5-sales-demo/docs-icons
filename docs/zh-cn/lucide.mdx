---
title: Lucide 图标
description: 'Lucide 图标精选参考——通过 Icon 组件可使用 1,600 余款现代描边图标。'
sidebar:
  label: Lucide
  order: 5
i18n:
...
[Lucide](https://lucide.dev/) 提供约 1,600 款现代、风格统一的描边图标。使用来自 `@f5xc-salesdemos/icons-lucide` 的 `Icon` 组件，可通过名称渲染任意图标。
...
图标以内联 SVG 形式渲染，使用 `currentColor` —— 可自动适配亮色和暗色模式。
...
如需查看完整的约 1,600 款 Lucide 图标列表，请访问 [lucide.dev/icons](https://lucide.dev/icons/)。

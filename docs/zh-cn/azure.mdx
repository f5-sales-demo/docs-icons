---
title: Azure 架构图标
description: Azure 架构图标精选参考 — 606 个彩色服务图标，用于云基础架构图和文档。
sidebar:
  label: Azure
  order: 15
i18n:
...
[Azure 架构图标](https://learn.microsoft.com/en-us/azure/architecture/icons/) 提供 606 个彩色服务图标，用于记录 Azure 基础架构。使用来自 `@f5xc-salesdemos/icons-azure` 的 `Icon` 组件，通过名称渲染任意图标。默认尺寸为 18x18，带有渐变填充。
...
这些图标使用原始供应商颜色（而非 `currentColor`）。颜色嵌入在 SVG 标记中，在浅色和深色模式下均得以保留。

---
title: Azure 架构图标
description: Azure 架构图标精选参考 — 606 个彩色服务图标，适用于云基础设施图表和文档。
sidebar:
  label: Azure
  order: 15
i18n:
...
[Azure 架构图标](https://learn.microsoft.com/en-us/azure/architecture/icons/) 提供了 606 个彩色服务图标，用于记录 Azure 基础设施。使用 `@f5xc-salesdemos/icons-azure` 中的 `Icon` 组件，可以按名称渲染任意图标。默认尺寸为 18x18，带有渐变填充。
...
这些图标使用原始供应商颜色（而非 `currentColor`）。颜色嵌入在 SVG 标记中，在浅色和深色模式下均保持不变。
...
如需查看完整的 606 个 Azure 架构图标列表，请访问 [Microsoft Azure 架构图标](https://learn.microsoft.com/en-us/azure/architecture/icons/) 页面。

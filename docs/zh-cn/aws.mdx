...
[AWS 架构图标](https://aws.amazon.com/architecture/icons/) 提供了 885 个彩色服务图标，用于记录 AWS 基础设施。使用 `@f5xc-salesdemos/icons-aws` 中的 `Icon` 组件，通过名称渲染任意图标。默认尺寸为 48x48，支持按图标单独覆盖尺寸。
...
这些图标使用供应商原始颜色（非 `currentColor`）。颜色嵌入在 SVG 标记中，在浅色和深色模式下均会保留。

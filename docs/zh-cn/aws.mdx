---
title: AWS 架构图标
description: AWS 架构图标精选参考 — 885 个彩色服务图标，适用于云基础设施图表和文档。
sidebar:
  label: AWS
  order: 14
i18n:
...
[AWS 架构图标](https://aws.amazon.com/architecture/icons/) 提供了 885 个彩色服务图标，用于记录 AWS 基础设施。使用 `@f5xc-salesdemos/icons-aws` 中的 `Icon` 组件，通过名称渲染任意图标。默认大小为 48x48，支持按图标单独覆盖大小。
...
如需查看全部 885 个 AWS 架构图标的完整列表，请访问 [AWS 架构图表图标](https://aws.amazon.com/architecture/icons/)。

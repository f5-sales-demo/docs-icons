...
[Carbon Icons](https://icon-sets.iconify.design/carbon/) 提供了 2,582 个来自 IBM 企业设计语言的简洁线框图标。使用 `@f5xc-salesdemos/icons-carbon` 中的 `Icon` 组件，通过名称渲染任意图标。
...
图标以内联 SVG 形式渲染，使用 `currentColor` — 它们会自动适配浅色和深色模式。

...
[Phosphor Icons](https://icon-sets.iconify.design/ph/) 提供 9,161 个图标，支持灵活的粗细选项 — regular、bold、duotone、fill、light 和 thin。使用 `@f5xc-salesdemos/icons-phosphor` 中的 `Icon` 组件，通过名称渲染任意图标。
...
图标以内联 SVG 形式渲染，使用 `currentColor` — 可自动适应浅色和深色模式。

:::note[粗细变体]
Phosphor 图标有六种粗细。regular 粗细使用基本名称（例如 `globe`）。其他粗细会附加后缀：`globe-bold`、`globe-duotone`、`globe-fill`、`globe-light`、`globe-thin`。以下示例展示的是 regular 粗细。
:::
...
如需查看完整的 9,161 个 Phosphor Icons（涵盖所有粗细变体），请访问 [Iconify Phosphor 浏览器](https://icon-sets.iconify.design/ph/)。

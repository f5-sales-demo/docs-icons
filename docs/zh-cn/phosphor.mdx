---
title: Phosphor 图标
description: 'Phosphor Icons 精选参考 — 提供 9,161 个图标，支持灵活的粗细变体，可通过 Icon 组件使用。'
sidebar:
  label: Phosphor
  order: 8
i18n:
...
[Phosphor Icons](https://icon-sets.iconify.design/ph/) 提供 9,161 个图标，支持灵活的粗细选项 — regular（常规）、bold（粗体）、duotone（双色调）、fill（填充）、light（细体）和 thin（极细）。使用 `@f5xc-salesdemos/icons-phosphor` 中的 `Icon` 组件，通过名称渲染任意图标。
...
图标以内联 SVG 形式渲染，使用 `currentColor` — 会自动适应浅色和深色模式。

:::note[粗细变体]
Phosphor 图标提供六种粗细。常规粗细使用基础名称（例如 `globe`）。其他粗细会附加后缀：`globe-bold`、`globe-duotone`、`globe-fill`、`globe-light`、`globe-thin`。以下示例展示的是常规粗细。
:::
...
如需查看完整的 9,161 个 Phosphor 图标（涵盖所有粗细变体），请访问 [Iconify Phosphor 浏览器](https://icon-sets.iconify.design/ph/)。

---
title: Phosphor 图标
description: 'Phosphor Icons 精选参考 — 9,161 个图标，提供灵活的粗细变体，可通过 Icon 组件使用。'
sidebar:
  label: Phosphor
  order: 8
i18n:
...
[Phosphor Icons](https://icon-sets.iconify.design/ph/) 提供 9,161 个图标，具有灵活的粗细选项 — regular、bold、duotone、fill、light 和 thin。使用 `@f5xc-salesdemos/icons-phosphor` 中的 `Icon` 组件，通过名称渲染任意图标。
...
:::note[粗细变体]
Phosphor 图标有六种粗细。regular 粗细使用基础名称（例如 `globe`）。其他粗细会附加后缀：`globe-bold`、`globe-duotone`、`globe-fill`、`globe-light`、`globe-thin`。以下示例展示的是 regular 粗细。
:::
...
如需查看全部 9,161 个 Phosphor 图标（涵盖所有粗细变体）的完整列表，请访问 [Iconify Phosphor 浏览器](https://icon-sets.iconify.design/ph/)。
